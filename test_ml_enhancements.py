...
def test_attack_style_classification():
    ...
    assert style1 in ['aggressive', 'counter-attacking', 'possession-heavy'], f"Expected attacking style, got {style1}"
    ...
    assert style2 in ['defensive', 'balanced'], f"Expected defensive style, got {style2}"
...
def test_referee_impact_model():
    ...
    print(f"  Z-score (penalties): {impact['statistical_profile']['z_penalties']:+.2f}σ")
    print(f"  Predicted cards: {impact['predictions']['expected_cards']}")
    print(f"  Cards CI: [{impact['predictions']['cards_confidence_interval']['lower']}-{impact['predictions']['cards_confidence_interval']['upper']}]")
...
def test_weather_regression():
    ...
    for i, weather in enumerate(conditions):
        ...
        print(f"  Condition {i+1}: Temp={weather['temperature']}°C, Rain={weather['precipitation']}mm, Wind={weather['wind_speed']}km/h")
        print(f"    Goal modifier: {result['goal_modifier']:.3f}")
        if 'regression_components' in result:
            print(f"    Components: temp={result['regression_components']['temperature_effect']:.3f}, "
...
def test_elo_system():
    ...
    real_madrid_elo = real_madrid_data.get('elo', 1500) if isinstance(real_madrid_data, dict) else real_madrid_data
    barcelona_elo = barcelona_data.get('elo', 1500) if isinstance(barcelona_data, dict) else barcelona_data
    unknown_elo = unknown_data.get('elo', 1500) if isinstance(unknown_data, dict) else unknown_data
    ...
    original_elo = original_data.get('elo', 1500) if isinstance(original_data, dict) else original_data
    ml.update_elo_after_match("Test Team", "Test Opponent", 3, 1)
    updated_data = ml.get_team_elo("Test Team")
    updated_elo = updated_data.get('elo', 1500) if isinstance(updated_data, dict) else updated_data
