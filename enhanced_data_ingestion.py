...
class EnhancedDataIngestion:
    ...
    def __init__(self, api_key: str, enable_flashscore: bool = True):
        ...
        if self.enable_flashscore:
            ...
            if enable_flashscore and not FLASHSCORE_AVAILABLE:
                logger.warning("[WARNING] FlashScore integration requested but not available")
    ...
    def _rate_limit(self):
        ...
        if elapsed < self.min_delay:
            sleep_time = min(self.max_delay, self.min_delay + (self.max_delay - self.min_delay) * 0.1)
    ...
    def fetch_flashscore_data(self, league: LeagueConfig, days_ahead: int = 30) -> Optional[Dict]:
        ...
        try:
            ...
            if fixture_env and Path(fixture_env).exists():
                ...
                matches = self.flashscore_scraper.get_league_matches(league.folder, days_ahead)
    ...
    def merge_data_sources(self, football_data: Dict, flashscore_data: Optional[Dict] = None) -> Dict:
        ...
        if flashscore_data:
            ...
            football_matches = {match['id']: match for match in football_data.get('matches', [])}
            ...
            for fs_match in flashscore_data.get('matches', []):
                fs_home = normalize(fs_match.get('home_team', '') or fs_match.get('home', ''))
                fs_away = normalize(fs_match.get('away_team', '') or fs_match.get('away', ''))
                ...
                for fd_match in football_matches.values():
                    ...
                    if (home_team == fs_home or fs_home in home_team or home_team in fs_home) and \
                       (away_team == fs_away or fs_away in away_team or away_team in fs_away):
            ...
            merged['flashscore_metadata'] = {
                'integrated': True,
                'matches_enhanced': len([m for m in football_data.get('matches', []) if 'flashscore_data' in m]),
                'live_scores_available': len(flashscore_data.get('live_scores', [])) > 0
            }

        merged['data_integration'] = {
            'sources': ['football-data.org'] + (['flashscore.es'] if flashscore_data else []),
    ...
    def _calculate_data_quality(self, data: Dict) -> float:
        ...
        if matches:
            complete_matches = sum(1 for m in matches if m.get('homeTeam') and m.get('awayTeam'))
    ...
    def save_enhanced_snapshot(self, league: LeagueConfig, season: int, data: Dict, force: bool = False) -> Path:
        ...
        data['snapshot_metadata'] = {
            ...
            'flashscore_integrated': data.get('flashscore_metadata', {}).get('integrated', False)
        }
        ...
        logger.info(f"[SAVE] Enhanced snapshot: {target} (Quality: {data['snapshot_metadata']['quality_score']:.1f}%)")
        return target

    def ingest_league_season(self, league: LeagueConfig, season: int, force: bool = False) -> bool:
        """Ingest complete season data from all available sources"""
        logger.info(f"[START] Starting enhanced ingestion for {league.name} season {season}")

        # Fetch Football-Data.org data
        football_data = self.fetch_football_data_matches(league, season)
        if not football_data:
            logger.error(f"[ERROR] Failed to fetch base data for {league.name} season {season}")
            return False
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    if args.live_only:
        if not FLASHSCORE_AVAILABLE or FlashScoreScraper is None:
            raise SystemExit("[ERROR] FlashScore integration required for live-only mode but not available.")
        ...
        for score in live_scores[:10]:  # Show first 10
            print(f"  • {score.get('home_team', 'Unknown')} vs {score.get('away_team', 'Unknown')}")
    ...
    logger.info(f"[START] Starting enhanced ingestion for {len(seasons)} season(s) of {args.league}")
    ...
    logger.info(f"[COMPLETE] Successfully ingested {success_count}/{len(seasons)} seasons for {args.league}")
