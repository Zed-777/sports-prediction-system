...
class Phase2LitePredictor:
    ...
    def enhanced_prediction(self, match_data: Dict[str, Any], competition_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            base_prediction = self.enhanced_predictor.enhanced_prediction(match_data, competition_code)

            # Step 2: Enhanced data quality assessment
            enhanced_data = self.data_quality_enhancer.comprehensive_data_enhancement(match_data)
            ...
            reliability_metrics = self.reliability_calculator.calculate(validated_prediction, enhanced_data)
            calibration_details = self.reliability_calculator.apply_calibration(validated_prediction, reliability_metrics)
            ...
            validated_prediction['confidence_intervals'] = reliability_metrics.get('confidence_intervals', {})
            ...
            base_accuracy = validated_prediction.get('report_accuracy_probability', 0.68)
            calibrated_accuracy = max(0.45, min(0.95, (base_accuracy * 0.6) + (reliability_score / 100.0 * 0.4)))
            validated_prediction['report_accuracy_probability'] = calibrated_accuracy
            validated_prediction.setdefault('calibration_details', {})['accuracy_adjustment'] = {
            ...
            validated_prediction.update({
                ...
                'confidence_assessment': self._assess_confidence_level(validated_prediction['confidence']),
                'data_quality_assessment': self._assess_data_quality(enhanced_data),
                'prediction_reliability': reliability_metrics
            })

            self.logger.info(f"Phase 2 Lite prediction completed in {validated_prediction['phase2_processing_time']:.3f}s")
    ...
    def _validate_and_enhance_confidence(self, base_prediction: Dict[str, Any],
        ...
        quality_boost = (data_quality - 75) / 100 * 0.2  # Up to 20% boost for excellent data
        enhancement_factors.append(('data_quality', quality_boost))

        # Factor 2: Match history depth (0-15% boost)
        home_matches = base_prediction.get('home_performance_analysis', {}).get('home', {}).get('matches', 0)
        away_matches = base_prediction.get('away_performance_analysis', {}).get('away', {}).get('matches', 0)
        total_matches = home_matches + away_matches

        history_boost = min(total_matches / 30, 1.0) * 0.15  # 15% boost for 30+ total matches
        enhancement_factors.append(('match_history', history_boost))

        # Factor 3: Head-to-head data (0-10% boost)
        h2h_meetings = base_prediction.get('head_to_head_analysis', {}).get('total_meetings', 0)
        ...
        enhanced_prediction['confidence_enhancement_factors'] = dict(enhancement_factors)
    ...
    def _calculate_consistency_boost(self, prediction: Dict[str, Any]) -> float:
        ...
        try:
            ...
            consistency = 1.0 - min(abs(goal_ratio - prob_ratio) / max(goal_ratio, prob_ratio), 1.0)
    ...
    def _assess_prediction_reliability(self, prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if reliability_score >= 85:
            ...
            recommendation = 'Highly reliable prediction - suitable for important decisions'
...
def test_phase2_lite():
    ...
    try:
        ...
        print(f"   Confidence: {result['confidence']:.1%} ({result['confidence_assessment']['level']})")
