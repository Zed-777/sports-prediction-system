...
def draw_overlay(image_path, out_path=None):
    ...
    for name, (x,y,w,h), color in regions:
        ...
        # label
        draw.text((rect[0]+6, rect[1]+6), f"{name} {x:.2f},{y:.2f}", font=font, fill=(0,0,0,220))
    ...
    for name, (cx, cy, r), color in gauges:
        ...
        draw.text((rect[0]+4, rect[1]+4), f"{name} c=({cx:.2f},{cy:.2f}) r={r:.2f}", font=font, fill=(0,0,0,220))

    # Column labels (win/draw/away) approximate boxes
    cols = [ (2.2, 13.2, 1.0, 0.8, 'home_col'), (5.0, 13.2, 1.0, 0.8, 'draw_col'), (7.8, 13.2, 1.0, 0.8, 'away_col') ]
