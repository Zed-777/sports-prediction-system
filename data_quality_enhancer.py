...
class DataQualityEnhancer:
    ...
    def __init__(self, football_api_key: str, weather_api_key: str | None = None):
        ...
        try:
            ...
            self._injuries_disabled_until = getattr(self, '_injuries_disabled_until', 0.0)
    ...
    def get_player_injury_impact(self, team_id: int, team_name: str) -> JSONDict:
        ...
        try:
            if os.path.exists(cache_file):
                ...
                ttl = int(self._settings.get('data_sources', {}).get('cache_ttl_by_endpoint', {}).get('v3.football.api-sports.io', {}).get('/v3/injuries', 86400))
    ...
    def _fetch_squad_data(self, team_id: int) -> Optional[JSONDict]:
        ...
        try:
            ...
            response = safe_request_get(url, headers=self.headers, timeout=10, retries=3, backoff=0.5, logger=None)
    ...
    def _fetch_injury_data_api_football(self, team_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if time.time() < getattr(self, '_injuries_disabled_until', 0.0) or getattr(self, 'skip_injuries', False):
                    return None
            ...
            response = safe_request_get(url, headers=headers, params=params, timeout=10, retries=3, backoff=0.5, logger=None)
            if response.status_code == 200:
                ...
                if response.status_code == 429:
                    ...
                    if getattr(self, 'injuries_disable_ttl_override', None):
                        ...
                        ttl = int(self._settings.get('data_sources', {}).get('disable_on_429_seconds', {}).get('api-football-v1.p.rapidapi.com', {}).get('/v3/injuries', 900))
                    ...
                    try:
                        ...
                        self._state_sync.set_disabled_flag('api-football-v1.p.rapidapi.com', '/v3/injuries', disabled_until, reason='429')
                    except Exception:
                        pass
                    # Backwards compatible: write a file used by older code/tests
                    try:
                        os.makedirs('data/cache', exist_ok=True)
                        with open('data/cache/injuries_disabled_until.json', 'w', encoding='utf-8') as f:
                            json.dump({'disabled_until': disabled_until, 'reason': '429'}, f)
                    except Exception:
                        pass
                    print("   Injuries endpoint temporary disabled for 15 minutes due to repeated rate limits")
                return None
    ...
    def _analyze_injury_data(self, injury_payload: JSONDict | list[JSONDict], team_name: str) -> JSONDict:
        ...
        injuries: list[JSONDict] = injury_payload if isinstance(injury_payload, list) else (injury_payload.get('injuries', []) if isinstance(injury_payload, dict) else [])
    ...
    def _analyze_squad_availability(self, squad_data: JSONDict, team_name: str) -> JSONDict:
        ...
        for player in squad:
            ...
            elif any(x in pos_lower for x in ['defence', 'defender', 'def', 'back', 'cb', 'lb', 'rb', 'lwb', 'rwb', 'center-back', 'centre-back', 'left-back', 'right-back']):
                position_normalized = 'Defender'
            elif any(x in pos_lower for x in ['midfielder', 'midfield', 'mid', 'cm', 'cdm', 'cam', 'lm', 'rm', 'dm', 'am', 'central midfield', 'defensive midfield', 'attacking midfield']):
                position_normalized = 'Midfielder'
            elif any(x in pos_lower for x in ['attacker', 'attack', 'offence', 'offense', 'forward', 'fw', 'striker', 'st', 'cf', 'lw', 'rw', 'winger', 'centre-forward', 'center-forward']):
    ...
    def calculate_strength_reduction(self, injury_data: JSONDict) -> float:
        ...
        per_player = float(self._settings.get('constants', {}).get('injury', {}).get('per_player_pct', 3.5))
        midfield_penalty = float(self._settings.get('constants', {}).get('injury', {}).get('midfield_penalty', 5))
        defense_penalty = float(self._settings.get('constants', {}).get('injury', {}).get('defense_penalty', 3))
        max_reduction = float(self._settings.get('constants', {}).get('injury', {}).get('max_reduction_pct', 25))
    ...
    def fetch_weather_data(self, city: str, match_datetime: str) -> JSONDict:
        ...
        cache_key = f"{city.lower()}::{match_datetime[:16].replace('T', '_').replace(':', '')}"
        ...
        try:
            ...
            resolved = self._fetch_open_meteo_weather(stadium_coordinates, match_datetime)
            if resolved:
                self._weather_cache[cache_key] = resolved
                print(f"🌤️ Weather for {city} at {match_datetime}: {resolved.get('temperature')}°C, {resolved.get('conditions')}")
                return resolved
        except Exception as e:
    ...
    def _fetch_open_meteo_weather(self, coords: Dict[str, Any], match_datetime: str) -> Optional[JSONDict]:
        ...
        try:
            ...
            if 'T' in match_datetime:
                ...
                match_dt = datetime.combine(match_day, datetime.min.time().replace(hour=match_hour))
            else:
        ...
        temp_values = _get_values_at_hours(hourly.get("temperature_2m", []), hour_indices)
        wind_values = _get_values_at_hours(hourly.get("wind_speed_10m", []), hour_indices)
        humidity_values = _get_values_at_hours(hourly.get("relative_humidity_2m", []), hour_indices)
        precip_values = _get_values_at_hours(hourly.get("precipitation", []), hour_indices)
        ...
        precipitation = sum(max(v, 0.0) for v in precip_values) if precip_values else 0.0
    ...
    def _get_stadium_coordinates(self, city: str) -> Dict[str, Any]:
        ...
        stadium_locations = {
            # Spain
            'madrid': {'lat': 40.4530, 'lon': -3.6883, 'altitude': 650, 'location_type': 'inland'},  # Santiago Bernabéu
            'barcelona': {'lat': 41.3809, 'lon': 2.1228, 'altitude': 12, 'location_type': 'coastal'},  # Camp Nou
            'seville': {'lat': 37.3840, 'lon': -5.9706, 'altitude': 7, 'location_type': 'inland'},
            'valencia': {'lat': 39.4747, 'lon': -0.3583, 'altitude': 15, 'location_type': 'coastal'},
            'bilbao': {'lat': 43.2641, 'lon': -2.9493, 'altitude': 19, 'location_type': 'coastal'},
            'vigo': {'lat': 42.2117, 'lon': -8.7394, 'altitude': 30, 'location_type': 'coastal'},
            'pamplona': {'lat': 42.7966, 'lon': -1.6378, 'altitude': 449, 'location_type': 'inland'},
            'laspalmas': {'lat': 28.1000, 'lon': -15.4550, 'altitude': 5, 'location_type': 'coastal'},
            'san sebastian': {'lat': 43.3013, 'lon': -1.9736, 'altitude': 10, 'location_type': 'coastal'},
            'villarreal': {'lat': 39.9440, 'lon': -0.1036, 'altitude': 43, 'location_type': 'inland'},
            'girona': {'lat': 41.9608, 'lon': 2.8287, 'altitude': 75, 'location_type': 'inland'},
            'getafe': {'lat': 40.3256, 'lon': -3.7147, 'altitude': 620, 'location_type': 'inland'},
            'valladolid': {'lat': 41.6437, 'lon': -4.7614, 'altitude': 693, 'location_type': 'inland'},
            'mallorca': {'lat': 39.5904, 'lon': 2.6320, 'altitude': 13, 'location_type': 'coastal'},
            'leganes': {'lat': 40.3563, 'lon': -3.7607, 'altitude': 665, 'location_type': 'inland'},
            'alaves': {'lat': 42.8370, 'lon': -2.6876, 'altitude': 512, 'location_type': 'inland'},
            'espanyol': {'lat': 41.3480, 'lon': 2.0755, 'altitude': 64, 'location_type': 'coastal'},
            'celta': {'lat': 42.2117, 'lon': -8.7394, 'altitude': 30, 'location_type': 'coastal'},
            'rayo vallecano': {'lat': 40.3919, 'lon': -3.6588, 'altitude': 650, 'location_type': 'inland'},
            # England
            'london': {'lat': 51.5550, 'lon': -0.1084, 'altitude': 35, 'location_type': 'inland'},  # Emirates/Spurs area
            'manchester': {'lat': 53.4631, 'lon': -2.2913, 'altitude': 38, 'location_type': 'inland'},  # Old Trafford
            'liverpool': {'lat': 53.4308, 'lon': -2.9608, 'altitude': 17, 'location_type': 'coastal'},
            'birmingham': {'lat': 52.5090, 'lon': -1.8847, 'altitude': 140, 'location_type': 'inland'},
            'newcastle': {'lat': 54.9756, 'lon': -1.6217, 'altitude': 48, 'location_type': 'coastal'},
            'brighton': {'lat': 50.8617, 'lon': -0.0844, 'altitude': 20, 'location_type': 'coastal'},
            'wolverhampton': {'lat': 52.5903, 'lon': -2.1304, 'altitude': 134, 'location_type': 'inland'},
            'leicester': {'lat': 52.6203, 'lon': -1.1423, 'altitude': 60, 'location_type': 'inland'},
            'southampton': {'lat': 50.9059, 'lon': -1.3910, 'altitude': 17, 'location_type': 'coastal'},
            'nottingham': {'lat': 52.9399, 'lon': -1.1328, 'altitude': 52, 'location_type': 'inland'},
            'bournemouth': {'lat': 50.7354, 'lon': -1.8383, 'altitude': 15, 'location_type': 'coastal'},
            'ipswich': {'lat': 52.0551, 'lon': 1.1453, 'altitude': 24, 'location_type': 'coastal'},
            # Germany
            'munich': {'lat': 48.2188, 'lon': 11.6247, 'altitude': 520, 'location_type': 'inland'},  # Allianz Arena
            'dortmund': {'lat': 51.4926, 'lon': 7.4518, 'altitude': 86, 'location_type': 'inland'},
            'berlin': {'lat': 52.5145, 'lon': 13.2394, 'altitude': 34, 'location_type': 'inland'},
            'frankfurt': {'lat': 50.0686, 'lon': 8.6455, 'altitude': 100, 'location_type': 'inland'},
            'leipzig': {'lat': 51.3459, 'lon': 12.3489, 'altitude': 113, 'location_type': 'inland'},
            'stuttgart': {'lat': 48.7923, 'lon': 9.2318, 'altitude': 247, 'location_type': 'inland'},
            'cologne': {'lat': 50.9336, 'lon': 6.8748, 'altitude': 53, 'location_type': 'inland'},
            # Italy
            'milan': {'lat': 45.4781, 'lon': 9.1240, 'altitude': 120, 'location_type': 'inland'},  # San Siro
            'rome': {'lat': 41.9341, 'lon': 12.4547, 'altitude': 37, 'location_type': 'inland'},
            'turin': {'lat': 45.1096, 'lon': 7.6412, 'altitude': 239, 'location_type': 'inland'},
            'naples': {'lat': 40.8279, 'lon': 14.1931, 'altitude': 17, 'location_type': 'coastal'},
            'florence': {'lat': 43.7808, 'lon': 11.2824, 'altitude': 50, 'location_type': 'inland'},
            'genoa': {'lat': 44.4156, 'lon': 8.9527, 'altitude': 20, 'location_type': 'coastal'},
            'bologna': {'lat': 44.4925, 'lon': 11.3096, 'altitude': 54, 'location_type': 'inland'},
            'verona': {'lat': 45.4353, 'lon': 10.9686, 'altitude': 59, 'location_type': 'inland'},
            'bergamo': {'lat': 45.7089, 'lon': 9.6808, 'altitude': 249, 'location_type': 'inland'},
            # France
            'paris': {'lat': 48.8414, 'lon': 2.2530, 'altitude': 35, 'location_type': 'inland'},  # Parc des Princes
            'marseille': {'lat': 43.2698, 'lon': 5.3959, 'altitude': 15, 'location_type': 'coastal'},
            'lyon': {'lat': 45.7654, 'lon': 4.9820, 'altitude': 173, 'location_type': 'inland'},
            'lille': {'lat': 50.6121, 'lon': 3.1305, 'altitude': 21, 'location_type': 'inland'},
            'monaco': {'lat': 43.7274, 'lon': 7.4157, 'altitude': 45, 'location_type': 'coastal'},
            'nice': {'lat': 43.7050, 'lon': 7.1926, 'altitude': 10, 'location_type': 'coastal'},
        }
        ...
        try:
            ...
            response = safe_request_get(geocode_url, params=params, timeout=5, logger=None)
            if response.status_code == 200:
                data = response.json()
                ...
                if results:
                    ...
                    print(f"📍 Geocoded '{city}' → {coords['lat']:.4f}, {coords['lon']:.4f}")
                    return coords
        except Exception as e:
    ...
    def _get_fallback_weather(self, city: str, match_date: str) -> JSONDict:
        ...
        try:
            month = int(match_date.split('-')[1]) if '-' in match_date else datetime.now().month
    ...
    def analyze_weather_impact(self, weather: JSONDict, stadium_info: Optional[JSONDict] | None = None, team_styles: Optional[JSONDict] | None = None) -> JSONDict:
        """
        ...
        if enforce_caps and caps:
            tmin, tmax = caps.get('temperature', {}).get('min', -20), caps.get('temperature', {}).get('max', 45)
            pmin, pmax = caps.get('precipitation', {}).get('min', 0), caps.get('precipitation', {}).get('max', 25)
            wmin, wmax = caps.get('wind_speed', {}).get('min', 0), caps.get('wind_speed', {}).get('max', 60)
            hmin, hmax = caps.get('humidity', {}).get('min', 10), caps.get('humidity', {}).get('max', 100)
            ...
            for var, val, vmin, vmax, name in [
                ...
                if val < vmin or val > vmax:
                    clamped = True
                    clamped_fields[name] = {'original': val, 'clamped_to': max(vmin, min(vmax, val))}
        ...
        temp_effect = math.exp(-((temperature - optimal_temp) ** 2) / (2 * temp_sigma ** 2))
        ...
        precip_modifier = math.exp(rain_coefficient * precipitation / (1 + rain_decay_rate * precipitation))
        ...
        humidity_modifier = 1.0 - 0.08 * (humidity_deviation ** 2)  # Max 8% reduction at extremes
        ...
        if stadium_info:
            ...
            if roof_type == 'retractable' and precipitation > 1.0:
                ...
                stadium_effects.append("Retractable roof closed - weather impact reduced")
        ...
        base_modifier = temp_modifier * precip_modifier * wind_modifier * humidity_modifier
        final_modifier = base_modifier * interaction_modifier * stadium_modifier * team_weather_modifier
    ...
    def _generate_tactical_adjustments(self, temp: float, precip: float, wind: float, humidity: float, modifier: float) -> list:
        ...
        if precip > 3.0:
            adjustments.extend(["Long ball tactics favored", "Increased goalkeeper errors possible"])
        elif precip > 1.0:
            adjustments.append("Direct play favored over possession")
        
        if wind > 30:
            adjustments.extend(["Aerial play severely affected", "Shooting accuracy reduced"])
        elif wind > 20:
    ...
    def _generate_conditions_summary(self, weather: Dict[str, Any], modifier: float) -> str:
    ...
    def _analyze_referee_impact(self, referee_stats: JSONDict, referee_name: str) -> JSONDict:
        """
        ...
        LEAGUE_BASELINES = {
            ...
            'home_bias_pct': {'mean': 52.5, 'std': 4.2},  # Slight natural home advantage
        ...
        if adjusted_impact >= 0.6:
            impact_level = 'high'
            impact_description = f'High influence expected ({adjusted_impact:.0%} probability)'
        elif adjusted_impact >= 0.35:
            impact_level = 'moderate'
            impact_description = f'Moderate influence expected ({adjusted_impact:.0%} probability)'
        else:
            impact_level = 'low'
            impact_description = f'Minimal influence expected ({adjusted_impact:.0%} probability)'
        ...
        if z_home_bias > 1.0:
            ...
            key_patterns.append(f'Home team advantage: {bias_strength} ({home_bias:.1f}% home-favorable decisions)')
        elif z_home_bias < -1.0:
            bias_strength = 'significant' if z_home_bias < -1.5 else 'moderate'
            key_patterns.append(f'Away team advantage: {bias_strength} ({home_bias:.1f}% home-favorable decisions)')
        else:
            key_patterns.append(f'Balanced officiating ({home_bias:.1f}% home decisions, within normal range)')
        ...
        if z_penalties > 1.0:
            ...
            key_patterns.append(f'Elevated penalty likelihood ({p_penalty:.0%} per match)')
        elif z_penalties < -1.0:
            p_penalty = 0.20 + (z_penalties * 0.05)
            key_patterns.append(f'Conservative penalty calls ({max(0.05, p_penalty):.0%} per match)')
        
        # Red card analysis
        if z_red > 1.5:
            key_patterns.append(f'Higher red card frequency ({red_cards:.2f}/match vs {LEAGUE_BASELINES["red_cards_per_game"]["mean"]:.2f} avg)')
        ...
        importance_factor = {'high': 1.15, 'normal': 1.0, 'low': 0.9}.get(match_importance, 1.0)
        ...
        predicted_cards = base_cards * importance_factor * derby_factor * conditions_factor
        ...
        return {
            ...
            'predictions': {
                ...
                'cards_confidence_interval': {'lower': round(cards_lower, 1), 'upper': round(cards_upper, 1)},
                'penalty_probability': round(min(1.0, penalty_rate), 3),
                'red_card_probability': round(min(1.0, red_cards * importance_factor * derby_factor), 3)
            },
            'context_factors': {
        ...
        try:
            ...
            for comp in competitions:
                ...
                try:
                    ...
                    params: dict[str, Union[str, int]] = {"status": "FINISHED", "limit": 50}
                    params = {k: str(v) for k, v in params.items()}
    ...
    def _analyze_real_referee_data(self, referee_name: str, matches: JSONList) -> Optional[JSONDict]:
    ...
    def _build_referee_analysis(self, stats: JSONDict, referee_name: str = 'Unknown') -> JSONDict:
        """
        ...
        confidence_score = 1.0 - math.exp(-matches_count / 15.0)  # Exponential approach to 1.0
        ...
        if predictions.get('penalty_probability', 0) > 0.30:
            key_tendencies.append(f"Elevated penalty likelihood: {predictions.get('penalty_probability', 0):.0%}")
    ...
    def _fetch_team_news_data(self, team_name: str, match_date: str) -> Optional[JSONDict]:
    ...
    def _analyze_team_news(self, news_data: Dict[str, Any], team_name: str) -> Dict[str, Any]:
    ...
    def comprehensive_data_enhancement(self, match: JSONDict, venue_info: Optional[JSONDict] = None) -> JSONDict:
        ...
        match_datetime = match_date_raw[:19] if len(match_date_raw) >= 19 else match_date_raw[:10]
        ...
        try:
            ...
            if home_data and isinstance(home_data, dict):
                ...
                if data_avail and squad_size > 15:  # Has real data
                    if key_injured == 0 and strength_loss < 5:
                        ...
                        quality_details.append("✅ Home team: Full squad (0 key injuries)")
                    elif key_injured <= 2 and strength_loss < 15:
                        quality_score += 16  # Good: minor injuries
                        quality_details.append(f"⚠️ Home team: {key_injured} key injuries")
                    else:
                        quality_score += 12  # Fair: some impact
                        quality_details.append(f"⚠️ Home team: {key_injured} injuries ({strength_loss}% impact)")
                elif data_avail:
            ...
            if away_data and isinstance(away_data, dict):
                ...
                if data_avail and squad_size > 15:  # Has real data
                    if key_injured == 0 and strength_loss < 5:
                        quality_score += 20  # Excellent: full squad available
                        quality_details.append("✅ Away team: Full squad (0 key injuries)")
                    elif key_injured <= 2 and strength_loss < 15:
                        quality_score += 16  # Good: minor injuries
                        quality_details.append(f"⚠️ Away team: {key_injured} key injuries")
                    else:
                        quality_score += 12  # Fair: some impact
                        quality_details.append(f"⚠️ Away team: {key_injured} injuries ({strength_loss}% impact)")
                elif data_avail:
                    quality_score += 8
            ...
            if weather_data and isinstance(weather_data, dict):
                ...
                has_real_conditions = cond_str and cond_str not in ['unknown', 'TBD', 'N/A', '', 'Unknown']
                ...
                if has_real_conditions and has_impact and has_goal_adj:
                    ...
                    quality_details.append(f"✅ Weather: {cond_str} (with impact analysis)")
            ...
            if referee_data and isinstance(referee_data, dict):
                ...
                if data_avail and referee_name not in ['Unknown Referee', 'TBD', '', 'N/A']:
                    if exp_level and strict_level and cards_per_game > 0:
                        quality_score += 20  # Complete referee profile
                        quality_details.append(f"✅ Referee: {referee_name} ({exp_level}, {cards_per_game} cards/game)")
                    elif exp_level and cards_per_game > 0:
                        quality_score += 16  # Good profile
                        quality_details.append(f"⚠️ Referee: {referee_name} ({exp_level})")
                    else:
            ...
            if home_news and isinstance(home_news, dict):
                ...
                if home_avail and formation and sentiment not in [None, 'unknown', '']:
                    ...
                    quality_details.append(f"✅ Home news: {formation} ({sentiment} sentiment)")
            ...
            if away_news and isinstance(away_news, dict):
                ...
                if away_avail and formation and sentiment not in [None, 'unknown', '']:
                    ...
                    quality_details.append(f"✅ Away news: {formation} ({sentiment} sentiment)")
            ...
            if flashscore_data and isinstance(flashscore_data, dict):
                ...
                if is_integrated:
                    ...
                    if true_sources >= 3:
                        ...
                        quality_details.append("✅ FlashScore: Full integration (stats+metrics+odds)")
                    elif true_sources >= 2:
            ...
            try:
                data_count = sum(1 for ds in data_sources if ds and isinstance(ds, dict) and len(ds) > 0)
                quality_score = 55 + min(25, data_count * 4)
                quality_details = [f"Fallback: {data_count}/{len(data_sources)} sources available"]
            except Exception:
                quality_score = 65.0
    ...
    def _infer_city_from_team(self, team_name: str) -> str:
        ...
        name_parts = team_name.replace(' FC', '').replace(' CF', '').replace(' SC', '').replace(' AFC', '').strip()
        ...
        if first_word and first_word[0].isupper() and first_word.lower() not in ['real', 'athletic', 'sporting', 'fc', 'cf', 'ac', 'as', 'ss', 'united', 'city', 'club']:
            return first_word
