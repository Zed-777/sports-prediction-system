...
class DataQualityEnhancer:
    ...
    def comprehensive_data_enhancement(self, match: Dict, venue_info: Optional[Dict] = None) -> Dict:
        ...
        home_team_obj = _get_field(match, 'homeTeam', 'home_team', {}) or {}
        away_team_obj = _get_field(match, 'awayTeam', 'away_team', {}) or {}
        ...
        home_team_name = _get_field(home_team_obj, 'name', 'team_name', '') or ''
        away_team_name = _get_field(away_team_obj, 'name', 'team_name', '') or ''

        match_date_raw = _get_field(match, 'utcDate', 'date', '') or ''
        ...
        if self.flashscore_integrator:
            try:
                league_key = self._get_league_key_from_match(match)
                enhanced_match = self.flashscore_integrator.enhance_match_data(match, league_key) or {}
                flashscore_data = {
