...
class EnhancedPredictor:
                    ...
                    for host, paths in endpoint_ttls.items():
                        for path_prefix, ttl in (paths or {}).items():
                            try:
                                ...
                                if (
                                    normalized_ep.endswith(normalized_prefix)
                                    or normalized_ep.startswith(normalized_prefix)
                                    or normalized_ep == normalized_prefix
                                ):
                                    return int(ttl)
                            except Exception:
                                pass
                    ...
                    for host, paths in endpoint_ttls.items():
                        ...
                        try:
                            ...
                            if cache_key.startswith("h2h"):
                                for host, paths in endpoint_ttls.items():
                                    ttl = (
                                        paths.get("/v4/matches/")
                                        if paths and "/v4/matches/" in paths
                                        else None
                                    )
                                    if ttl:
                                        return int(ttl)
    ...
    def _cleanup_old_cache(self) -> None:
        ...
        for cache_info in cache_files:
            ...
            if (
                cache_info["age"] > 86400  # 24 hours old
                or cache_info["access_age"] > 21600
            ):  # Not accessed in 6 hours
                try:
    ...
    def analyze_head_to_head(
        ...
        return {
            ...
            "avg_goals_for_when_home": total_goals_for_when_home
            / max(matches_as_home, 1),
            "avg_goals_against_when_home": total_goals_against_when_home
            / max(matches_as_home, 1),
            "avg_goals_for_when_away": total_goals_for_when_away
            / max(matches_as_away, 1),
            "avg_goals_against_when_away": total_goals_against_when_away
            / max(matches_as_away, 1),
            "recent_form": recent_form,  # Last 5 meetings
            "home_advantage_vs_opponent": (wins_when_home / max(matches_as_home, 1))
            * 100,
    ...
    def analyze_head_to_head_enhanced(
        ...
        matches_as_home = sum(
            1 for m in sorted_matches if m["homeTeam"]["id"] == home_team_id
        )
        matches_as_away = total_matches - matches_as_home
        ...
        prev_points = (
            sum(3 if r == "W" else (1 if r == "D" else 0) for r in prev_3)
            if prev_3
            else recent_points
        )
        ...
        momentum_trend = (
            ...
            else ("Declining" if recent_points < prev_points else "Stable")
        )
        ...
        return {
            ...
            * 100,
            ...
            "recent_dominance": recent_points
            / max(len(recent_3) * 3, 1)
            * 100,  # % of max points in recent meetings
            "h2h_quality": (
                "HIGH"
                if total_matches >= 8
                else ("MEDIUM" if total_matches >= 4 else "LOW")
            ),
            "venue_performance": venue_performance,
            "weighted_win_rate": (wins_when_home + wins_when_away)
            / max(total_matches, 1)
            * 100,
        }
    ...
    def fetch_team_home_away_stats(
        ...
        try:
            ...
            stats = {
                ...
                "home_advantage": home_performance.get("win_rate", 0)
                - away_performance.get("win_rate", 0),
                "scoring_difference": home_performance.get("avg_goals_for", 0)
                - away_performance.get("avg_goals_for", 0),
            }
    ...
    def calculate_performance_stats(self, stats: JSONDict) -> JSONDict:
        ...
        if "recent_matches" in stats:
            ...
            basic_stats["recent_matches"] = stats["recent_matches"][
                :5
            ]  # Last 5 for reports
    ...
    def _calculate_weighted_form(self, recent_matches: JSONList) -> JSONDict:
        ...
        if total_weight > 0:
            weighted_form_score = (
                total_weighted_points / (total_weight * max_possible)
            ) * 100
        else:
            weighted_form_score = 50.0  # Neutral default
        ...
        return {
            ...
            "recent_5_performance": round(
                sum(momentum_trend[:5]) / max(1, min(5, len(momentum_trend))) / 3 * 100,
                1,
            ),
            ...
            "confidence_interval": {
                "lower": round(form_lower, 1),
                "upper": round(form_upper, 1),
            },
    ...
    def predict_goal_timing(
        ...
        for period, base_prob in timing_profile.items():
            ...
            if (
                home_attack_style == "counter-attacking"
                or away_attack_style == "counter-attacking"
            ):
            ...
            if (
                home_attack_style == "possession-heavy"
                or away_attack_style == "possession-heavy"
            ):
        ...
        first_half_prob = (
            sum(adjusted_timing[p] for p in ["0-15", "16-30", "31-45"]) * 100
        )
        second_half_prob = (
            sum(adjusted_timing[p] for p in ["46-60", "61-75", "76-90"]) * 100
        )
        ...
        period_xg = {
            period: prob * total_xg for period, prob in adjusted_timing.items()
        }
        ...
        return {
            ...
            "period_probabilities": {
                p: round(v * 100, 1) for p, v in adjusted_timing.items()
            },
            ...
            "scoring_tempo": (
                "high" if total_xg > 3.0 else ("medium" if total_xg > 2.0 else "low")
            ),
        }
    ...
    def _classify_attack_style(
        ...
        avg_goals_for = (
            home_data.get("avg_goals_for", 1.3) + away_data.get("avg_goals_for", 1.2)
        ) / 2
        avg_goals_against = (
            home_data.get("avg_goals_against", 1.2)
            + away_data.get("avg_goals_against", 1.3)
        ) / 2
    ...
    def calculate_expected_score(
        ...
        home_attack = max(
            0.8,
            (
                home_stats["home"]["avg_goals_for"]
                if home_stats["home"]["matches"] > 0
                else 1.35
            ),
        )
        home_defense = max(
            0.7,
            (
                home_stats["home"]["avg_goals_against"]
                if home_stats["home"]["matches"] > 0
                else 1.25
            ),
        )
        away_attack = max(
            0.8,
            (
                away_stats["away"]["avg_goals_for"]
                if away_stats["away"]["matches"] > 0
                else 1.25
            ),
        )
        away_defense = max(
            0.7,
            (
                away_stats["away"]["avg_goals_against"]
                if away_stats["away"]["matches"] > 0
                else 1.35
            ),
        )
        ...
        if h2h_data["total_meetings"] > 0:
            ...
            home_attack = (
                home_attack * (1 - h2h_weight)
                + h2h_data["avg_goals_for_when_home"] * h2h_weight
            )
            away_attack = (
                away_attack * (1 - h2h_weight)
                + h2h_data["avg_goals_for_when_away"] * h2h_weight
            )
        ...
        home_expected = (
            home_attacking_strength * away_defensive_strength * (league_avg / 2)
        ) + home_advantage
        away_expected = (
            away_attacking_strength * home_defensive_strength * (league_avg / 2)
        )
        ...
        home_score_probs = [
            (i, poisson_prob(i, home_expected)) for i in range(MAX_GOALS)
        ]
        away_score_probs = [
            (i, poisson_prob(i, away_expected)) for i in range(MAX_GOALS)
        ]
        ...
        return {
            ...
            "score_probabilities": [
                (f"{score[0]}-{score[1]}", prob * 100)
                for score, prob in common_scores[:5]
            ],
            "score_probabilities_normalized": [
                (
                    f"{score[0]}-{score[1]}",
                    self.normalize_probability_to_10_scale(prob * 100),
                )
                for score, prob in common_scores[:5]
            ],
    ...
    def _reconcile_xg_with_win_probs(
        self,
        home_xg: float,
        away_xg: float,
        home_win_prob: float,
    ...
    def calculate_over_under_prob(self, total_expected: float, line: float) -> float:
        ...
        for i in range(int(line) + 1):
            prob_under += (
                total_expected**i * math.exp(-total_expected)
            ) / math.factorial(i)
    ...
    def enhanced_prediction(self, match: JSONDict, competition_code: str) -> JSONDict:
        ...
        if flashscore_signals:
            try:
                ...
                if isinstance(flashscore_signals, dict):
                    ...
                    # Live score handling
                    fs_score = (
                        flashscore_signals.get("score")
                        or flashscore_signals.get("scores")
                        or {}
                    )
                    home_lead = None
                    ...
                    if (
                        home_lead is None
                        and isinstance(fs_score, (list, tuple))
                        and len(fs_score) >= 2
                    ):
                        try:
                            _hs_val = fs_score[0]
                    ...
                    try:
                        ...
                        team_stats = (
                            flashscore_signals.get("team_statistics")
                            if isinstance(flashscore_signals, dict)
                            else None
                        )
                        if team_stats and isinstance(team_stats, dict):
                            ...
                            fs_lineup_home = (
                                h_stats.get("expected_lineup_strength")
                                or h_stats.get("lineup_strength")
                                or h_stats.get("strength_rating")
                            )
                            fs_lineup_away = (
                                a_stats.get("expected_lineup_strength")
                                or a_stats.get("lineup_strength")
                                or a_stats.get("strength_rating")
                            )
                    ...
                    try:
                        odds = (
                            ...
                            or flashscore_signals.get("odds")
                            or {}
                        )
                        if isinstance(odds, dict):
                            fs_odds_home = (
                                odds.get("home") or odds.get("h") or odds.get("1")
                            )
                            fs_odds_away = (
                                odds.get("away") or odds.get("a") or odds.get("2")
                            )
                            # sometimes odds are nested in market lists
                            if (
                                not fs_odds_home or not fs_odds_away
                            ) and "markets" in odds:
                                m = odds.get("markets")
                        ...
                        if (
                            ...
                            flashscore_note = (
                                (flashscore_note or "")
                                + f" | Market odds available ({fs_odds_home:.2f}/{fs_odds_away:.2f})"
                            )
                    except Exception:
                        pass
        ...
        home_form = (
            home_stats["home"]["win_rate"] if home_stats["home"]["matches"] > 0 else 50
        )
        away_form = (
            away_stats["away"]["win_rate"] if away_stats["away"]["matches"] > 0 else 30
        )
        ...
        form_adjustment_home = (
            home_form - 50
        ) * 0.15  # Reduced weight since form is in xG
        ...
        if h2h_data.get("total_meetings", 0) >= 3:
            ...
            try:
                ...
                hadv_f = 50.0
            h2h_home_boost = (
                (hadv_f - 50.0) * h2h_weight * 0.5
            )  # Reduced since xG already has H2H
        ...
        try:
            fs_feats = (
                match.get("_flashscore_features", {}) if isinstance(match, dict) else {}
            )
            if fs_feats:
                fs_home_shift = (
                    float(fs_feats.get("fs_home_shift", 0.0))
                    if fs_feats.get("fs_home_shift") is not None
                    else 0.0
                )
                fs_away_shift = (
                    float(fs_feats.get("fs_away_shift", 0.0))
                    if fs_feats.get("fs_away_shift") is not None
                    else 0.0
                )
        ...
        if (
            home_stats.get("home", {}).get("matches", 0) >= 5
            and away_stats.get("away", {}).get("matches", 0) >= 5
        ):
        ...
        try:
            ...
            match_cache_key = (
                f"{home_team_id}_{away_team_id}_{full_datetime.replace(':', '')}"
            )
            ...
            if not weather_data:
                ...
                match_datetime = match.get("utcDate", "")[
                    :19
                ]  # Include time for hourly accuracy
            ...
            if ai_result and ai_result.get("ai_features_active", False):
                ...
                final_pred = (
                    ai_result.get("final_prediction")
                    if ai_result.get("final_prediction") is not None
                    else {}
                )
        ...
        final_home_win_prob = (
            poisson_home_prob * blend_weight_poisson
            + home_win_prob * (1 - blend_weight_poisson)
        )
        final_draw_prob = poisson_draw_prob * blend_weight_poisson + draw_prob * (
            1 - blend_weight_poisson
        )
        final_away_win_prob = (
            poisson_away_prob * blend_weight_poisson
            + away_win_prob * (1 - blend_weight_poisson)
        )
        ...
        result = {
            ...
            "score_probability_normalized": score_prediction_reconciled[
                "score_probability_normalized"
            ],
            "top3_combined_probability": score_prediction_reconciled[
                "top3_combined_probability"
            ],
            "top3_probability_normalized": score_prediction_reconciled[
                "top3_probability_normalized"
            ],
            ...
            "score_probabilities_normalized": score_prediction_reconciled[
                "score_probabilities_normalized"
            ],
            "over_2_5_probability": score_prediction_reconciled["over_2_5_probability"],
            "both_teams_score_probability": score_prediction_reconciled[
            ...
            and hasattr(self, "ai_ml_predictor")
            and self.ai_ml_predictor is not None,
            "market_intelligence_active": self.market_intelligence_available
            ...
            "match_context": (
                final_pred.get("match_context", "unknown")
                if "final_pred" in locals()
                else "unknown"
            ),
            "model_agreement_factor": (
                final_pred.get("model_agreement_factor", 0.5)
                if "final_pred" in locals()
                else 0.5
            ),
            "optimization_applied": (
                final_pred.get("optimization_applied", False)
                if "final_pred" in locals()
                else False
            ),
            "component_weights": (
                final_pred.get("component_weights", {})
                if "final_pred" in locals()
                else {}
            ),
        }
        ...
        if market_analysis:
            try:
                result["betting_market_analysis"] = (
                    self._convert_market_analysis_to_dict(market_analysis)
                )
            except Exception as e:
        ...
        if self.advanced_stats is not None:
            try:
                ...
                if stats_analysis.get("analysis_applied"):
                    result["expected_home_goals"] = stats_analysis[
                        "adjusted_home_goals"
                    ]
                    result["expected_away_goals"] = stats_analysis[
                        "adjusted_away_goals"
                    ]
                    result["shot_quality"] = {
        ...
        if self.player_impact is not None:
            try:
                ...
                impact_analysis = self.player_impact.analyze_match_impact(
                    ...
                    away_prob=result["away_win_prob"],
                    home_unavailable=(
                        home_missing if isinstance(home_missing, list) else []
                    ),
                    away_unavailable=(
                        away_missing if isinstance(away_missing, list) else []
                    ),
                )
    ...
    def ai_enhanced_prediction(
        ...
        try:
            ...
            try:
                avg_home_goals = (
                    h2h_data.get("avg_goals_for_when_home")
                    if h2h_data.get("avg_goals_for_when_home") is not None
                    else 1.5
                )
                avg_away_goals = (
                    h2h_data.get("avg_goals_for_when_away")
                    if h2h_data.get("avg_goals_for_when_away") is not None
                    else 1.3
                )
            ...
            if self.ai_statistics:
                ...
                self.ai_statistics.monte_carlo_iterations = (
                    5000  # Reduced from 10000 for speed
                )
            ...
            form_consistency = (
                momentum_data["home_momentum_score"]
                + momentum_data["away_momentum_score"]
            ) / 2
            ...
            if self.ai_ml_predictor:
                ...
                prediction_result["accuracy_estimate"] = (
                    0.65 + (data_quality * 0.2) + (h2h_quality * 0.1)
                )
        ...
        if (
            ...
            try:
                ...
                data_timestamps = {
                    "team_stats_age_seconds": current_time
                    - match_data.get("_home_stats_timestamp", match_time),
                    "h2h_data_age_seconds": current_time
                    - h2h_data.get("_fetch_timestamp", match_time),
                    "injury_data_age_seconds": current_time
                    - referee_data.get("_injury_timestamp", match_time),
                    "form_data_age_seconds": current_time
                    - home_stats.get("_fetch_timestamp", match_time),
                    "weather_data_age_seconds": current_time
                    - weather_data.get("_fetch_timestamp", match_time),
                }
        ...
        if prediction_result.get("final_prediction"):
            try:
                ...
                bayesian_adjusted, bayesian_meta = (
                    self.bayesian_updater.adjust_confidence(league_adjusted)
                )
                ...
                try:
                    ...
                    context_adjusted, context_meta = (
                        self.context_extractor.apply_all_context_adjustments(
                    ...
                    phase3_metadata["context_adjustment"] = {
                ...
                prediction_result["_phase3_adjustment"] = {
                    ...
                    "total_adjustment_factor": (
                        final_confidence / confidence_before_phase3
                        if confidence_before_phase3 > 0
                        else 1.0
                    ),
                    "league": league,
    ...
    def _infer_city_from_team(self, team_name: str) -> str:
        ...
        if (
            ...
            not in ["real", "athletic", "sporting", "fc", "cf", "ac", "as", "ss"]
        ):
            return first_word

        return "Madrid"  # Ultimate fallback

    def _generate_legacy_prediction(
        self,
        match_data: JSONDict,
        home_stats: JSONDict,
        away_stats: JSONDict,
        ...
        # Home advantage
        base_home_prob = (
            0.45 + (home_win_rate - away_win_rate) * 0.3 + (home_form - away_form) * 0.2
        )
        base_away_prob = (
            0.30
            + (away_win_rate - home_win_rate) * 0.2
            + (away_form - home_form) * 0.15
        )
        base_draw_prob = 0.25
        ...
        try:
            fs_feats = (
                match_data.get("flashscore_features", {})
            ...
            if isinstance(odds, dict):
                try:
                    ...
                    total = sum(
                        x
                        for x in (market_home, market_draw, market_away)
                        if x is not None
                    )
                    if total and total > 0:
                        mh = (
                            (market_home / total) * 100
                            if market_home is not None
                            else 0.0
                        )
                        md = (
                            (market_draw / total) * 100
                            if market_draw is not None
                            else 0.0
                        )
                        ma = (
                            (market_away / total) * 100
    ...
    def _calibrate_probs_nonlinear(
        ...
        for c in ["legacy", "ml", "neural", "monte_carlo"]:
            ...
            if (
                p is not None
            ):  # FIXED: Check for None explicitly, not truthiness (allows 0.0)
        ...
        remaining = 1.0 - cal_h
        draw_prob = (
            float(probs.get("draw_probability"))
            if probs.get("draw_probability") is not None
            else 0.33
        )
        away_prob = (
            float(probs.get("away_win_probability"))
            if probs.get("away_win_probability") is not None
            else 0.33
        )
    ...
    def _create_ai_ensemble_prediction(
        ...
        ml: JSONDict,
        neural: JSONDict,
        monte_carlo: JSONDict,
        tactical_patterns: JSONDict,
    ) -> JSONDict:
        """Create weighted ensemble with adaptive optimization (IMPROVED)"""
        home_xg = (
            legacy.get("expected_home_goals", 1.5) * 0.3
            + ml.get("expected_home_goals", 1.5) * 0.4
            + neural.get("neural_goals_home", 1.0) * 0.3
        )
        away_xg = (
            legacy.get("expected_away_goals", 1.3) * 0.3
            + ml.get("expected_away_goals", 1.3) * 0.4
        ...
        ml_home = (
            _to_decimal(_safe_get(ml, "home_win_probability", legacy_home * 100))
            if has_ml
            else legacy_home
        )
        ml_draw = (
            _to_decimal(_safe_get(ml, "draw_probability", legacy_draw * 100))
            if has_ml
            else legacy_draw
        )
        ml_away = (
            _to_decimal(_safe_get(ml, "away_win_probability", legacy_away * 100))
            if has_ml
            else legacy_away
        )

        neural_home = (
            _to_decimal(_safe_get(neural, "neural_home_prob", legacy_home * 100))
            if has_neural
            else legacy_home
        )
        neural_draw = (
            _to_decimal(_safe_get(neural, "neural_draw_prob", legacy_draw * 100))
            if has_neural
            else legacy_draw
        )
        neural_away = (
            _to_decimal(_safe_get(neural, "neural_away_prob", legacy_away * 100))
            if has_neural
            else legacy_away
        )
        ...
        mc_draw = (
            _to_decimal(_safe_get(monte_carlo, "draw_probability", legacy_draw * 100))
            if has_mc
            else legacy_draw
        )
        mc_away = (
        ...
        cal_probs = self._calibrate_probs_nonlinear(
            {
                "home_win_probability": home_prob_raw,
                "draw_probability": draw_prob_raw,
                "away_win_probability": away_prob_raw,
            },
            active_comps,
        )
        ...
        ml_home_goals = (
            _safe_get(ml, "expected_home_goals", legacy_home_goals)
            if has_ml
            else legacy_home_goals
        )
        ml_away_goals = (
            _safe_get(ml, "expected_away_goals", legacy_away_goals)
            if has_ml
            else legacy_away_goals
        )
        neural_home_goals = (
            _safe_get(neural, "neural_goals_home", legacy_home_goals)
            if has_neural
            else legacy_home_goals
        )
        neural_away_goals = (
            _safe_get(neural, "neural_goals_away", legacy_away_goals)
            if has_neural
            else legacy_away_goals
        )
        mc_home_goals = (
            _safe_get(monte_carlo, "expected_home_goals", legacy_home_goals)
            if has_mc
            else legacy_home_goals
        )
        mc_away_goals = (
            _safe_get(monte_carlo, "expected_away_goals", legacy_away_goals)
            if has_mc
            else legacy_away_goals
        )

        home_goals = (
            legacy_home_goals * weights.get("legacy", 0.25)
            + ml_home_goals * weights.get("ml", 0.30)
            + neural_home_goals * weights.get("neural", 0.25)
            + mc_home_goals * weights.get("monte_carlo", 0.20)
        )
        away_goals = (
            legacy_away_goals * weights.get("legacy", 0.25)
        ...
        base_confidence = sum(
            ...
            for c, w in zip(
                ...
                [
                    weights.get(k, 0.25)
                    for k in ["legacy", "ml", "neural", "monte_carlo"]
                ],
                strict=True,
            )
        )
        ...
        ) * 0.15  # ±7.5% swing based on agreement
    ...
    def _serialize_market_odds(self, market_odds: MarketOdds | None) -> JSONDict:
        ...
        fetched_iso = (
            datetime.utcfromtimestamp(market_odds.fetched_at).isoformat() + "Z"
        )
        return {
            "available": True,
            "source": market_odds.source,
            ...
            "probabilities_percent": {
                k: round(v * 100, 2) for k, v in market_odds.probabilities.items()
            },
    ...
    def _apply_market_adjustment(
        ...
        adjusted["market_reference"] = {
            k: float(market_probabilities.get(k, 0.0)) for k in ("home", "draw", "away")
        }
