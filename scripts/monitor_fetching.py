...
def main():
    if not METRICS.exists():
        print('No metrics file found. Run fetch scripts with metrics flag to create fetch_metrics.json')
        return
    ...
    for provider, pv in data.items():
        ...
        print(f"Provider: {provider} - Total: {total} - 403: {errors_403} - 429: {errors_429}")
        if errors_403 > 0:
            print('  -> WARNING: Permission errors observed: consider changing subscription or keys')
        if errors_429 > 0:
            print('  -> ALERT: Rate limits observed. Consider longer backoff or schedule frequency lower')
