...
def process_matches(matches: list[dict]) -> tuple[list[dict], list[int]]:
    ...
    for m in matches_sorted:
        ...
        # Features
        features = {
            "home_strength": (home_stats.home_wins + home_stats.home_matches)
            / max(1, home_stats.home_matches + 1),
            "away_strength": (away_stats.away_wins + away_stats.away_matches)
            / max(1, away_stats.away_matches + 1),
...
def main():
    ...
    for sample, label in zip(all_processed, all_labels, strict=True):
        key = (
            sample.get("match_id")
            or f"{sample.get('date')}_{sample.get('home_team')}_{sample.get('away_team')}"
        )
        if key in unique:
            # skip duplicates
            continue
