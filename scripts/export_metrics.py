...
def export_metrics_to_json(out_dir: str) -> str:
    ...
    filename = os.path.join(out_dir, f"metrics_{datetime.utcnow().strftime('%Y%m%dT%H%M%S%fZ')}.json")
...
def export_metrics_to_csv(out_dir: str) -> str:
    os.makedirs(out_dir, exist_ok=True)
    metrics = get_metrics()
    filename = os.path.join(out_dir, f"metrics_{datetime.utcnow().strftime('%Y%m%dT%H%M%S%fZ')}.csv")
...
def collect_run_metadata() -> dict:
    ...
    try:
        meta['commit_sha'] = subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD'], stderr=subprocess.DEVNULL).decode().strip()
    except Exception:
    ...
    meta['run_id'] = os.getenv('GITHUB_RUN_ID') or os.getenv('CI_RUN_ID') or str(uuid.uuid4())
