...
def write_csv(rows: list[dict], out_path: Path):
    fieldnames = ['id', 'home_team', 'away_team', 'date', 'home_score', 'away_score', 'status']
...
def parse_args():
    ...
    parser.add_argument('--leagues', required=True, help='Comma-separated league ids (e.g., 140)')
    parser.add_argument('--seasons', required=True, help='Comma-separated seasons (years) e.g. 2019,2020')
    parser.add_argument('--outfile', help='Optional combined CSV path to write all matches into')
...
def main():
    ...
    if not api_key:
        print('No API_FOOTBALL_KEY found in environment or .env. Set it and re-run to fetch data.')
        sys.exit(0)

    leagues = [league_str.strip() for league_str in args.leagues.split(',') if league_str.strip()]
    seasons = [int(s.strip()) for s in args.seasons.split(',') if s.strip()]

    all_rows = []
    for league in leagues:
        for season in seasons:
            print(f'Fetching API-Football fixtures: league {league}, season {season}...')
            try:
                ...
                print(f'  -> failed to fetch fixtures for league {league} season {season}: {e}')
