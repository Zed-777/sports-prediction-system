...
class AccuracyOptimizer:
    ...
    def run_backtest(self, 
                     league: str = 'la-liga',
                     days_back: int = 90,
                     parameter_overrides: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        """
        ...
        historical_file = self.project_root / 'data' / 'historical' / f'{league}_results.json'
    ...
    def _simulate_backtest(self, 
        ...
        if params:
            ...
            # Total effect
            accuracy_adjustment = market_effect + k_effect + decay_effect + h2h_effect + poisson_effect
        ...
        return {
            ...
            'metrics': {
                ...
                'home_accuracy': round(estimated_accuracy + 0.02, 4),  # Usually better at home predictions
                'away_accuracy': round(estimated_accuracy - 0.03, 4),
                'draw_accuracy': round(estimated_accuracy - 0.08, 4),  # Draws hardest to predict
                'confidence_calibration': round(0.85 + (estimated_accuracy - 0.58) * 0.5, 4),
                'brier_score': round(0.25 - (estimated_accuracy - 0.50) * 0.2, 4),  # Lower is better
            },
    ...
    def run_parameter_sweep(self, 
                           param_name: str,
                           league: str = 'la-liga') -> Dict[str, Any]:
        """
        ...
        if param_name not in OPTIMIZABLE_PARAMS:
            raise ValueError(f"Unknown parameter: {param_name}. Available: {list(OPTIMIZABLE_PARAMS.keys())}")
        
        param = OPTIMIZABLE_PARAMS[param_name]
        logger.info(f"Running parameter sweep for {param_name}")
        logger.info(f"  Range: {param.min_value} to {param.max_value}, step {param.step}")
        ...
        # Save results
        output_file = self.results_dir / f'sweep_{param_name}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    ...
    def full_optimization(self, league: str = 'la-liga') -> Dict[str, Any]:
        ...
        # Save full results
        output_file = self.results_dir / f'full_optimization_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(optimization_result, f, indent=2)
        ...
        for change in optimization_result['recommended_changes']:
            print(f"  {change['parameter']}: {change['current']} → {change['recommended']}")
...
def main():
    ...
    parser.add_argument('--mode', choices=['backtest', 'sweep', 'experiment', 'full'],
                       default='backtest', help='Optimization mode')
    parser.add_argument('--param', type=str, help='Parameter to optimize (for sweep/experiment)')
    parser.add_argument('--value', type=float, help='Value to test (for experiment)')
    parser.add_argument('--league', type=str, default='la-liga', help='League to test on')
