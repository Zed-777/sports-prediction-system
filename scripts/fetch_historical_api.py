...
def parse_cli_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="PD",
        help="FD competition code or AF league ID",
    )
    parser.add_argument(
        "--seasons",
        type=str,
    ...
    parser.add_argument(
        "--incremental",
        action="store_true",
        help="Only fetch matches after last processed date",
    )
...
def main() -> None:
                ...
                try:
                    ...
                    for m in matches:
                        if m.get("status") == "finished" and (
                            m.get("home_score") is None or m.get("away_score") is None
                        ):
                            try:
                    ...
                    if matches:
                        ...
                        last.setdefault("football_data", {})[args.competition] = (
                            max_date
                        )
    ...
    out_file = (
        args.outfile or f"{args.competition}_seasons_{'_'.join(args.seasons)}.csv"
    )
