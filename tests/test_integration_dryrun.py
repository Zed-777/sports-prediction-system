...
def test_generate_dryrun(monkeypatch, tmp_path):
    ...
    gen = generate_fast_reports.SingleMatchGenerator.__new__(generate_fast_reports.SingleMatchGenerator)
    # Minimal attributes expected by generate_matches_report
    gen.api_key = 'test'
    gen.headers = {'X-Auth-Token': 'test'}
    gen._settings = getattr(generate_fast_reports.SingleMatchGenerator(), '_settings', {})
