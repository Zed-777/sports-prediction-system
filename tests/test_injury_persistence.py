...
def test_injury_disable_persistent_on_429(monkeypatch):
    ...
    def fake_safe_get(url, headers=None, params=None, timeout=None, retries=None, backoff=None, logger=None):
    ...
    assert enhancer._injuries_disabled_until == 0.0 or enhancer._injuries_disabled_until < time.time()
...
def test_injury_cache_written_on_success(monkeypatch):
    ...
    sample_payload = {'response': [{'player': {'name': 'John Doe'}, 'reason': 'knee', 'status': 'out'}]}
