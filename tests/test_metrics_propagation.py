...
def test_enhanced_predictor_metrics_propagation(monkeypatch):
    ...
    def fake_safe_get(url, headers=None, params=None, timeout=None, logger=None, **kwargs):
        payload = {"matches": [{"homeTeam": {"id": 1}, "awayTeam": {"id": 2}, "score": {"fullTime": {"home": 1, "away": 1}}}]}
        return SimpleNamespace(status_code=200, text='{}', json=lambda: payload, raise_for_status=lambda: None)
