...
def test_flashscore_ingest_and_image_generation(monkeypatch, tmp_path):
    ...
    match_data = {
        ...
        'player_availability': {'home_team': {'expected_lineup_strength': 90}, 'away_team': {'expected_lineup_strength': 88}},
...
def test_flashscore_merge_and_prediction(monkeypatch):
    ...
    monkeypatch.setattr(predictor, 'fetch_team_home_away_stats', lambda team_id, code: {
        'home': {'matches': 10, 'win_rate': 50, 'avg_goals_for': 1.2, 'avg_goals_against': 1.1},
        'away': {'matches': 8, 'win_rate': 40, 'avg_goals_for': 1.0, 'avg_goals_against': 1.3}
    })
    monkeypatch.setattr(predictor, 'fetch_head_to_head_history', lambda h, a, c: {
        'total_meetings': 2,
