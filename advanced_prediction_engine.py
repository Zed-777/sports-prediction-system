...
class AdvancedPredictionEngine:
    ...
    def __init__(self):
        ...
        self.team_ratings = {
            ...
            'Real Betis Balompié': {'attack': 0.72, 'defense': 0.68, 'form_weight': 0.72},
            'Sevilla FC': {'attack': 0.75, 'defense': 0.78, 'form_weight': 0.78},
            'Valencia CF': {'attack': 0.68, 'defense': 0.70, 'form_weight': 0.65},
            'RCD Espanyol de Barcelona': {'attack': 0.55, 'defense': 0.60, 'form_weight': 0.58},
    ...
    def calculate_advanced_probabilities(self, home_team, away_team, match_data):
        ...
        # Get team strengths
        home_ratings = self.team_ratings.get(home_team, {'attack': 0.55, 'defense': 0.55})
        away_ratings = self.team_ratings.get(away_team, {'attack': 0.55, 'defense': 0.55})
        ...
        home_advantage = self.venue_advantages.get(venue, self.venue_advantages['default'])

        # Adjust for team-specific home advantage (some teams play better at home)
        if home_team in ['Athletic Bilbao', 'Real Sociedad']:  # Known for strong home records
            home_advantage *= 1.2
        elif home_team in ['Valencia CF', 'RCD Espanyol de Barcelona']:  # Weaker home advantage
            home_advantage *= 0.8
    ...
    def calculate_expected_goals(self, home_team, away_team, probabilities):
        ...
        home_expected = (la_liga_avg_goals / 2) * (home_attack_factor / away_defense_factor) * 1.1  # Home boost

        # Away team expected goals
        away_attack_factor = away_ratings['attack']
        home_defense_factor = home_ratings['defense']
        away_expected = (la_liga_avg_goals / 2) * (away_attack_factor / home_defense_factor) * 0.9  # Away penalty
        ...
        for match in upcoming_matches:
            ...
            prediction = {
                ...
                'advanced_metrics': {
                    ...
                    'strength_difference': abs(self.get_team_strength(home_team) - self.get_team_strength(away_team)),
                    'home_advantage_applied': self.venue_advantages.get(match.get('venue', 'default'), 0.15),
                    'predicted_total_goals': home_goals + away_goals,
                    'match_competitiveness': 1 - abs(probabilities['home_win_prob'] - probabilities['away_win_prob'])
                }
            }
            ...
            print(f"     Probabilities: {probabilities['home_win_prob']:.1%} | {probabilities['draw_prob']:.1%} | {probabilities['away_win_prob']:.1%}")
    ...
    def generate_key_factors(self, home_team, away_team, match, confidence_breakdown):
        ...
        if home_strength > away_strength + 0.15:
            factors.append(f'{home_team} significantly stronger (Rating diff: {(home_strength-away_strength):.2f})')
        elif away_strength > home_strength + 0.15:
            factors.append(f'{away_team} significantly stronger (Rating diff: {(away_strength-home_strength):.2f})')
        else:
        ...
        if venue in self.venue_advantages and self.venue_advantages[venue] > 0.2:
            factors.append(f'Strong home advantage at {venue} (+{int(self.venue_advantages[venue]*100)}%)')
