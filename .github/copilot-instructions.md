...
This file gives concise, actionable guidance to an AI assistant working in this repository so you can be productive immediately.

High-level architecture
- The repo centers on a prediction engine (Phase 2 Lite) with three layers:
  - Data ingestion & validation: `enhanced_data_ingestion.py`, `data_quality_enhancer.py`, `app/data/`
  - Modeling/prediction: `enhanced_predictor.py`, `phase2_lite.py`, `phase2_integration.py`, `app/models/`
  - Reporting/CLI: `generate_fast_reports.py`, `generate_advanced_professional_reports.py`, `app/cli.py`, `run.py`

Key workflows & entrypoints
- Quick local run of Phase 2 Lite (sanity check):
  - `python phase2_lite.py` (this runs the Phase 2 Lite prediction harness)
- Generate reports: `python generate_fast_reports.py` or `python generate_advanced_professional_reports.py` depending on required output
- CLI: `app/cli.py` and `app/run.py` wire up routines for ingestion, training, and prediction; prefer using these for integrated flows

Dependencies & setup
- Use the repo top-level requirements files. Typical setup (PowerShell):
  ```powershell
  python -m venv .venv; .\.venv\Scripts\Activate.ps1; pip install -r requirements.txt
  ```
- Phase-2-specific dependencies: `requirements_phase2.txt` (install when working on Phase 2 Lite features)
...
- To add a new ingestion connector: follow `enhanced_data_ingestion.py` conventions, write to `data/raw/`, update `data_quality_enhancer.py` to register validations.
- To add a model: add code under `app/models/`, write a small CLI flag in `app/cli.py` and add a quick smoke test under `tests/`.

What to avoid / repository-specific gotchas
- Don't assume a single monolithic CLI — multiple entrypoints exist (root scripts + `app/cli.py`). Use `app/config.py` and `config/settings.yaml` for global toggles.
- Many files exist in `legacy_files/` and `BACKUP_v4.0_WORKING/` — prefer current top-level modules (`enhanced_predictor.py`, `phase2_lite.py`) when implementing features.

If you make changes
- Run the small test suite before committing: `python -m pytest -q` and run Phase 2 smoke: `python phase2_lite.py`.
- Keep edits small and add a test file in `tests/` for new behavior. Reference existing tests like `test_enhanced_ingestion.py`.

Questions for maintainer
- Are there any CI steps not present in `README.md` that should be included? (e.g., exact pytest subsets or linting commands)
- Is `app/run.py` the canonical production entrypoint or should the root `run.py` be used?

If anything in this file is unclear or you want deeper examples (small patch + test), tell me which area to expand.
