...
class ModelTrainer:
    ...
    def train_model(self, 
        ...
        try:
            ...
            if not historical_data or not historical_data.get('matches'):
                logger.error(f"No historical data available for training {league} model")
                return {
            ...
            results = {
                ...
                'training_date': datetime.now().isoformat(),
                'model_version': '1.0.0',
                'features_used': features or ['team_form', 'home_advantage', 'player_availability'],
            ...
            logger.info(f"Model training completed for {league} with {len(training_data)} samples")
            return results
    ...
    def _prepare_training_data(self, historical_data: Dict[str, Any], features: Optional[List[str]]) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Prepared {len(training_data)} training samples with features: {feature_cols}")
