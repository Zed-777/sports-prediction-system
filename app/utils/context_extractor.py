...
class ContextExtractor:
    ...
    def calculate_home_away_adjustment(
        ...
        """
        home_factors = self.HOME_AWAY_FACTORS.get(home_team_level, self.HOME_AWAY_FACTORS['average'])
        away_factors = self.HOME_AWAY_FACTORS.get(away_team_level, self.HOME_AWAY_FACTORS['average'])
    ...
    def calculate_competition_level_adjustment(
        self,
        home_level: str = 'mid_table',
        away_level: str = 'mid_table'
    ) -> Tuple[float, Dict]:
        """
        ...
        Returns:
            Tuple of (adjustment_factor, metadata)
        """
        home_comp = self.COMPETITION_LEVEL.get(home_level, self.COMPETITION_LEVEL['mid_table'])
    ...
    def record_match_at_venue(
        ...
        self.venue_performance[home_key]['samples'] = self.venue_performance[home_key]['total']
        ...
        self.venue_performance[away_key]['samples'] = self.venue_performance[away_key]['total']
    ...
    def apply_all_context_adjustments(
        ...
        cl_factor, cl_meta = self.calculate_competition_level_adjustment(comp_level, opp_level)
        adjusted *= cl_factor
        all_metadata['adjustments']['competition_level'] = cl_meta
        
        # 4. Venue adjustment (if data available)
        if venue and team:
            v_factor, v_meta = self.calculate_venue_performance_adjustment(venue, team, is_home)
            adjusted *= v_factor
        ...
        all_metadata['overall_factor'] = adjusted / confidence if confidence > 0 else 1.0
