...
class AdvancedFeatures:
    ...
    def calculate_referee_bias(self, match_data: Dict[str, Any], 
                              historical_referee_data: Optional[Dict] = None) -> float:
        """
        ...
        try:
            ...
            bias = (home_penalty_rate - away_penalty_rate) * 10 + (away_card_rate - home_card_rate) * 0.05
    ...
    def calculate_market_movement(self, match_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if opening_odds_home > 0:
                odds_movement = (opening_odds_home - current_odds_home) / opening_odds_home
            else:
                odds_movement = 0.0
    ...
    def calculate_venue_performance(self, match_data: Dict[str, Any],
                                   venue_history: Optional[Dict] = None) -> float:
        """
    ...
    # ========== PUBLIC API ==========
    def extract_all_features(self, match_data: Dict[str, Any],
                            historical_referee_data: Optional[Dict] = None,
                            venue_history: Optional[Dict] = None) -> Dict[str, float]:
        """
        ...
        features = {
            ...
            'referee_bias': self.calculate_referee_bias(match_data, historical_referee_data),
            'set_piece_efficiency': self.calculate_set_piece_efficiency(match_data),
            'shot_accuracy': self.calculate_shot_accuracy(match_data),
            'weather_impact': self.calculate_weather_impact(match_data.get('weather_data', {})),
            'market_movement': self.calculate_market_movement(match_data),
            'venue_performance': self.calculate_venue_performance(match_data, venue_history)
        }
        
        return features
    
    def extract_as_array(self, match_data: Dict[str, Any],
                        historical_referee_data: Optional[Dict] = None,
                        venue_history: Optional[Dict] = None) -> np.ndarray:
        """Extract features as numpy array (for ML models)"""
        features = self.extract_all_features(match_data, historical_referee_data, venue_history)
