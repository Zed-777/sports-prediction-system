...
class SmartDataValidator:
    ...
    def _load_validation_rules(self) -> dict[str, Any]:
        ...
        return {
            ...
            "form_scores": {"range": (0.0, 100.0), "typical_range": (20.0, 80.0)},
            "confidence_bounds": {
                "min_confidence": 0.3,
                "max_confidence": 0.95,
    ...
    def comprehensive_validation(
        ...
        is_valid = (
            overall_score >= 50.0 and len([i for i in issues if "CRITICAL" in i]) == 0
        )
    ...
    def _validate_schema(
        ...
        missing_fields = [
            field for field in required_fields if not match_data.get(field)
        ]
        if missing_fields:
            issues.append(
    ...
    def _validate_statistical(
        ...
        expected_avg = (
            league_context.get("typical_goals_per_match", 2.7) / 2
        )  # Per team

        # Calculate z-scores (simplified)
        home_z = abs(home_goals - expected_avg) / (
            expected_avg * 0.5
        )  # Assume std = 50% of mean
        ...
        home_form = (
            match_data.get("home_performance_analysis", {})
            .get("home", {})
            .get("weighted_form_score", 50)
        )
        away_form = (
            match_data.get("away_performance_analysis", {})
            .get("away", {})
            .get("weighted_form_score", 50)
        )
    ...
    def _validate_temporal(
        ...
        preferred_matches = self.validation_rules["temporal_constraints"][
            "preferred_matches"
        ]
    ...
    def validate_and_enhance_confidence(
        self,
        match_data: dict[str, Any],
