...
class AdaptiveAdjuster:
    ...
    def adapt_bayesian_parameters(self, calibration_error: float, samples: int) -> None:
        ...
        if error_ratio > 1.2:  # Error is 20%+ above baseline
            self.bayesian_adaptation['learning_rate_adjustment'] *= (1.0 - self.adaptation_rate * 0.2)
        elif error_ratio < 0.8:  # Error is 20%+ below baseline
            self.bayesian_adaptation['learning_rate_adjustment'] *= (1.0 + self.adaptation_rate * 0.1)
        
        # Clamp adjustment
        self.bayesian_adaptation['learning_rate_adjustment'] = max(0.5, min(1.5, 
    ...
    def save_adjuster_state(self) -> None:
        ...
        state = {
            ...
            'adaptation_history': self.adaptation_history[-100:],  # Keep last 100 events
            'timestamp': datetime.now().isoformat()
        }
        ...
        if os.path.exists(filepath):
            try:
                with open(filepath, 'r') as f:
                    state = json.load(f)
                    self.league_factors = state.get('league_factors', self.league_factors)
                    self.context_weights = state.get('context_weights', self.context_weights)
                    self.bayesian_adaptation = state.get('bayesian_adaptation', self.bayesian_adaptation)
                    self.confidence_scale = state.get('confidence_scale', self.confidence_scale)
