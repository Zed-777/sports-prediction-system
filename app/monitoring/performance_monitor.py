...
class PerformanceMonitor:
    ...
    def record_prediction(self, league: str, model: str, confidence: float, 
                          outcome: float, timestamp: Optional[datetime] = None) -> None:
        """
        ...
        # Record prediction
        is_correct = 1.0 if (confidence > 0.5 and outcome > 0.5) or (confidence <= 0.5 and outcome <= 0.5) else 0.0
        calibration_error = abs(confidence - outcome)
        
        # Update league metrics
        league_stats = self.league_performance[league]
        league_stats['accuracy'] = (league_stats['accuracy'] * league_stats['samples'] + is_correct) / (league_stats['samples'] + 1)
        league_stats['calibration_error'] = (league_stats['calibration_error'] * league_stats['samples'] + calibration_error) / (league_stats['samples'] + 1)
        ...
        model_stats['accuracy'] = (model_stats['accuracy'] * model_stats['samples'] + is_correct) / (model_stats['samples'] + 1)
    ...
    def _update_system_metrics(self, confidence: float, is_correct: float, calibration_error: float) -> None:
    ...
    def get_league_performance(self, league: str) -> Dict:
        ...
        return self.league_performance.get(league, {
            'accuracy': 0.0,
            'calibration_error': 0.0,
            'samples': 0
        })
    ...
    def get_drift_status(self) -> Tuple[bool, float]:
        ...
        """
        return self.system_metrics['drift_detected'], self.system_metrics['drift_severity']
    ...
    def get_recommendations(self) -> List[str]:
        ...
        accuracy_drop = self.baselines['accuracy'] - self.system_metrics['overall_accuracy']
        if accuracy_drop > 0.03:
            recommendations.append(f"⚠️ Accuracy dropped {accuracy_drop*100:.1f}% below baseline - recalibrate models")
        
        # Check for calibration drift
        calibration_increase = self.system_metrics['overall_calibration_error'] - self.baselines['calibration']
        if calibration_increase > 0.02:
            recommendations.append(f"⚠️ Calibration error increased {calibration_increase*100:.1f}% - rerun isotonic regression")
        
        # Check per-league performance
        for league, stats in self.league_performance.items():
            if stats['samples'] >= 5 and stats['accuracy'] < 0.55:
                recommendations.append(f"⚠️ {league}: Low accuracy ({stats['accuracy']*100:.1f}%) - review league tuning")
        
        # Check per-model performance
        for model, stats in self.model_performance.items():
            if stats['samples'] >= 5 and stats['accuracy'] < 0.50:
                recommendations.append(f"⚠️ {model}: Poor performance ({stats['accuracy']*100:.1f}%) - consider model retraining")
    ...
    def get_recent_window(self, league: str, window_size: Optional[int] = None) -> List[Tuple[float, float]]:
        """
...
class DriftAnalyzer:
    ...
    def analyze_drift(self, predictions: List[Tuple[float, float]]) -> Dict:
        ...
        reference = predictions[-self.reference_window_size - self.test_window_size:-self.test_window_size]
        current = predictions[-self.test_window_size:]
        
        # Calculate mean accuracy for each window
        ref_accuracy = sum(1 if (pred > 0.5) == (outcome > 0.5) else 0 
                          for pred, outcome in reference) / len(reference)
        curr_accuracy = sum(1 if (pred > 0.5) == (outcome > 0.5) else 0 
                           for pred, outcome in current) / len(current)
