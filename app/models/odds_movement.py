...
class OddsMovementTracker:
        ...
        if match_id not in self._movements:
            ...
            if isinstance(movement, dict):
                ...
                movement = OddsMovement(
                    ...
                    away_team=away_team,
                    opening_odds=(
                        OddsSnapshot.from_dict(movement["opening_odds"])
                        if movement.get("opening_odds")
                        else snapshot
                    ),
                    current_odds=snapshot,
                    snapshots=[],
    ...
    def _analyze_movement(self, movement: OddsMovement) -> OddsMovement:
        ...
        if home_strength.value == "sharp" or away_strength.value == "sharp":
            ...
            home_strength.value == "significant" or away_strength.value == "significant"
        ):
        ...
        if total_implied > 0:
            movement.implied_home_prob = (
                self._odds_to_prob(current.home_odds) / total_implied
            )
            movement.implied_draw_prob = (
                self._odds_to_prob(current.draw_odds) / total_implied
            )
            movement.implied_away_prob = (
                self._odds_to_prob(current.away_odds) / total_implied
            )

        # Determine market favorite
        if (
            current.home_odds < current.away_odds
            and current.home_odds < current.draw_odds
        ):
            movement.market_favorite = "home"
        elif (
            current.away_odds < current.home_odds
            and current.away_odds < current.draw_odds
        ):
    ...
    def _detect_sharp_money(self, movement: OddsMovement) -> OddsMovement:
        ...
        if (
            movement.home_movement == MovementDirection.SHORTENING
            and home_movement > self.SHARP_MOVEMENT_THRESHOLD
        ):
            sharp_detected = True
            sharp_side = "home"
        ...
        # Check for sharp movement on away
            ...
            movement.away_movement == MovementDirection.SHORTENING
            and away_movement > self.SHARP_MOVEMENT_THRESHOLD
        ):
            sharp_detected = True
            sharp_side = "away"
        ...
        if (
            movement.home_movement == MovementDirection.DRIFTING
            and movement.market_favorite == "home"
        ):
    ...
    def adjust_predictions(
        ...
        return (
            round(blended_home, 4),
            round(blended_draw, 4),
            round(blended_away, 4),
            reasons,
        )
