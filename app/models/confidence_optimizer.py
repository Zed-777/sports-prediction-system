...
class ConfidenceOptimizer:
    ...
    def _assess_data_sufficiency(
        ...
        home_matches = (
            prediction_data.get("home_performance_analysis", {})
            .get("home", {})
            .get("matches", 0)
        )
        away_matches = (
            prediction_data.get("away_performance_analysis", {})
            .get("away", {})
            .get("matches", 0)
        )
    ...
    def _apply_historical_calibration(
        ...
        calibration_factor = (
            historical_accuracy / base_confidence if base_confidence > 0 else 1.0
        )
    ...
    def _calculate_uncertainty_bounds(
        ...
        # Total uncertainty
        total_uncertainty = (
            base_uncertainty + agreement_uncertainty + sufficiency_uncertainty
        )
    ...
    def _integrate_confidence_factors(
        self,
        calibrated_confidence: float,
    ...
    def update_historical_performance(
        ...
        bucket_data["accuracy"] = (
            bucket_data["correct_predictions"] / bucket_data["total_predictions"]
        )
    ...
    def get_confidence_recommendation(
        ...
        if final_confidence >= 0.8:
            recommendations["recommendation"] = (
                "High confidence prediction - suitable for important decisions"
            )
            recommendations["reliability_assessment"] = "Very Reliable"
        elif final_confidence >= 0.6:
            recommendations["recommendation"] = (
                "Moderate confidence prediction - good for general analysis"
            )
            recommendations["reliability_assessment"] = "Reliable"
        else:
            recommendations["recommendation"] = (
                "Low confidence prediction - use with caution"
            )
