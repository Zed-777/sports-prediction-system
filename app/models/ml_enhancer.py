...
class MachineLearningEnhancer:
    ...
    def get_team_elo(self, team_name: str) -> dict:
        ...
        if team_name not in self.elo_ratings:
            ...
            for known_team in self.elo_ratings:
                if (
                    team_name.lower() in known_team.lower()
                    or known_team.lower() in team_name.lower()
                ):
    ...
    def update_elo_after_match(
        ...
        home_k = (
            self.K_FACTOR_NEW
            if home_rating["matches"] < 30
            else self.K_FACTOR_ESTABLISHED
        )
        away_k = (
            self.K_FACTOR_NEW
            if away_rating["matches"] < 30
            else self.K_FACTOR_ESTABLISHED
        )
        ...
        home_attack_change = (
            home_k * 0.5 * (home_goals / max(1, home_goals + away_goals) - 0.5)
        )
        away_attack_change = (
            away_k * 0.5 * (away_goals / max(1, home_goals + away_goals) - 0.5)
        )

        # Update defense ELO based on goals conceded (inverted)
        home_defense_change = (
            home_k * 0.5 * (0.5 - away_goals / max(1, home_goals + away_goals))
        )
        away_defense_change = (
            away_k * 0.5 * (0.5 - home_goals / max(1, home_goals + away_goals))
        )
    ...
    def train_from_processed_dataset(
        ...
        for p in processed:
            ...
            f = p.get("features", {})
            match["home_form"] = {
                "win_rate": f.get("home_recent_form", 0.5),
                "goals_per_game": f.get("home_avg_goals", 1.0),
                "goals_conceded_per_game": 1.0,
            }
            match["away_form"] = {
                "win_rate": f.get("away_recent_form", 0.5),
                "goals_per_game": f.get("away_avg_goals", 1.0),
                "goals_conceded_per_game": 1.0,
            }
            match["h2h"] = f.get("h2h", {})
    ...
    def _generate_simulated_training_data(
        ...
        for _i in range(num_matches):
            ...
            home_strength = (
                self.bayesian_ratings[home_team]["attack"]
                + self.bayesian_ratings[home_team]["defense"]
            ) / 2
            away_strength = (
                self.bayesian_ratings[away_team]["attack"]
                + self.bayesian_ratings[away_team]["defense"]
            ) / 2
