...
class BayesianUpdater:
    ...
    def record_match(
        ...
        prediction_correct = (predicted_home_win and home_won) or (not predicted_home_win and not home_won)
    ...
    def get_posterior_credible_interval(self, credibility: float = 0.95) -> Tuple[float, float]:
        """
    ...
    def adjust_confidence(self, raw_confidence: float) -> Tuple[float, Dict]:
        ...
        metadata = {
            ...
            'adjustment_factor': adjusted / raw_confidence if raw_confidence > 0 else 1.0,
    ...
    def get_bayesian_statistics(self) -> Dict:
        ...
        recent_accuracy = sum(1 for m in recent_history if m['prediction_correct']) / len(recent_history) if recent_history else 0.0
        
        return {
            'total_predictions': self.total_predictions,
            'successful_predictions': self.successful_predictions,
            'overall_accuracy': self.successful_predictions / self.total_predictions if self.total_predictions > 0 else 0.0,
            ...
            'ready_for_adjustment': self.total_predictions >= self.min_samples_for_adjustment,
    ...
    def save_bayesian_state(self) -> None:
        ...
        try:
            with open(os.path.join(self.cache_dir, 'bayesian_updater_state.json'), 'w') as f:
