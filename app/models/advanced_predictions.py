...
class TwoStagePredictionModel:
    ...
    def __init__(self):
        ...
        self.home_win_scores = {
            (1, 0): 0.28, (2, 0): 0.18, (2, 1): 0.22, (3, 0): 0.08,
            (3, 1): 0.10, (3, 2): 0.06, (4, 0): 0.02, (4, 1): 0.03,
            (4, 2): 0.02, (4, 3): 0.01
        }
        self.draw_scores = {
            (0, 0): 0.30, (1, 1): 0.38, (2, 2): 0.22, (3, 3): 0.08,
            (4, 4): 0.02
        }
        self.away_win_scores = {
            (0, 1): 0.30, (0, 2): 0.20, (1, 2): 0.22, (0, 3): 0.08,
            (1, 3): 0.10, (2, 3): 0.05, (0, 4): 0.02, (1, 4): 0.02,
            (2, 4): 0.01
        }
    ...
    def predict_score(
        ...
        alternatives = [(score, round(prob * 100, 1)) for score, prob in sorted_scores[1:4]]
        ...
        **kwargs
        ...
        return {
            ...
            'score': {
                ...
                'alternatives': [f"{s[0]}-{s[1]} ({p}%)" for (s, p) in score.alternative_scores]
            },
    ...
    def calculate_score_matrix(
        ...
        for home in range(self.MAX_GOALS + 1):
            for away in range(self.MAX_GOALS + 1):
                prob = self._poisson_prob(home, expected_home_goals) * \
...
if __name__ == "__main__":
    ...
    print(f"  {result['outcome']['home_prob']}% / {result['outcome']['draw_prob']}% / {result['outcome']['away_prob']}%")
    print(f"  Predicted: {result['outcome']['predicted']} ({result['outcome']['confidence']}% confidence)")
    print()
    print("Score Prediction:")
    print(f"  Most likely: {result['predicted_score']['most_likely']} ({result['predicted_score']['probability']}%)")
    print(f"  Alternatives: {result['predicted_score']['alternatives']}")
    print()
    print("BTTS Prediction:")
    print(f"  Yes: {result['btts']['btts_yes_probability']}% / No: {result['btts']['btts_no_probability']}%")
    print(f"  Prediction: {result['btts']['prediction']}")
    print()
    print("Over/Under 2.5:")
    ou_25 = result['over_under']['lines']['2.5']
    print(f"  Over: {ou_25['over_probability']}% / Under: {ou_25['under_probability']}%")
