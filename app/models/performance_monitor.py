...
class PerformanceMonitor:
    ...
    def __init__(self, cache_dir: str = 'data/cache', lookback_windows: List[int] = None):
        """
        ...
        self.league_performance: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            'total_predictions': 0,
    ...
    def record_prediction(self, league: str, prediction: Dict[str, Any]) -> None:
        ...
        try:
            ...
            record = {
                ...
                'outcome': prediction.get('outcome'),  # 0.0 (away win) to 1.0 (home win)
    ...
    def _get_league_performance(self, league: str) -> Dict[str, Any]:
        ...
        for window in self.lookback_windows:
            ...
            if recent:
                correct = sum(1 for p in recent if self._check_prediction_correctness(
                    p['confidence'], p['outcome']
                ))
    ...
    def detect_performance_drift(self, league: str, window: int = 50) -> Dict[str, Any]:
        ...
        result = {
            ...
            'recommendation': self._get_drift_recommendation(drift_magnitude, second_accuracy)
        }
    ...
    def suggest_recalibration_actions(self, league: str) -> List[str]:
        ...
        if stats.get('expected_calibration_error', 0.0) > 0.10:
            actions.append("High calibration error: Run isotonic regression recalibration")
        ...
        if window_accs:
            ...
            if recent_acc < 0.45:
                actions.append("Recent accuracy dropping: Review model weights and thresholds")
        ...
        if stats.get('total_predictions', 0) < 20:
            actions.append("Insufficient historical data: Continue collecting predictions")
        ...
        try:
            ...
            for league, stats in data.get('league_performance', {}).items():
                self.league_performance[league] = {
                    'total_predictions': stats['total_predictions'],
                    'correct_predictions': stats['correct_predictions'],
                    'confidence_scores': deque(stats.get('confidence_scores', []), maxlen=100),
                    'drift_detected': stats.get('drift_detected', False),
                    'last_recalibration': stats.get('last_recalibration')
                }
            
            self.logger.debug(f"Loaded performance history for {len(self.league_performance)} leagues")
    ...
    def _format_league_report(self, league: str, performance: Dict) -> List[str]:
        ...
        lines.append(f"  Overall Accuracy: {performance.get('overall_accuracy', 0):.1%}")
        lines.append(f"  Calibration Error: {performance.get('expected_calibration_error', 0):.4f}")
