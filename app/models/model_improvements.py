    ...
    def analyze_ensemble(
        ...
        if len(predictions) < 2:
            return {
                ...
                'warnings': [],
                'predicted_outcome': predictions[0].predicted_outcome if predictions else '1'
            }
        ...
        if max_std < self.STRONG_AGREEMENT_THRESHOLD and agreement_ratio >= 0.8:
            ...
            warnings.append(f"SEVERE model disagreement (std={max_std:.2f}) - high uncertainty")
...
class IsotonicCalibrator:
        ...
        if self.calibration_file.exists():
            try:
                with open(self.calibration_file, 'r') as f:
                    data = json.load(f)
                self._calibration_map = {int(k): v for k, v in data.get('map', {}).items()}
                self._prediction_counts = {int(k): v for k, v in data.get('counts', {}).items()}
                logger.info(f"Loaded calibration data with {len(self._calibration_map)} buckets")
    ...
    def add_outcome(
        ...
        self._calibration_map[bucket] = (current * count + (1.0 if was_correct else 0.0)) / (count + 1)
    ...
    def calibrate(self, probability: float) -> float:
        ...
        lower_bucket = max(k for k in self._calibration_map.keys() if k <= bucket) if any(k <= bucket for k in self._calibration_map.keys()) else None
        upper_bucket = min(k for k in self._calibration_map.keys() if k >= bucket) if any(k >= bucket for k in self._calibration_map.keys()) else None
        
        if lower_bucket is not None and upper_bucket is not None and lower_bucket != upper_bucket:
...
class ModelEnhancementSuite:
    ...
    def enhance_prediction(
        ...
        if abs(calibrated_prob - winner_prob) > 2:
            ...
            enhancements.append(f"Calibration adjustment: {winner_prob:.0f}% -> {calibrated_prob:.0f}%")
...
if __name__ == "__main__":
    ...
    print(f"  Upset alert: {result['upset_alert']} ({result['upset_probability']:.1f}%)")
