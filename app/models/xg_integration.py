...
class XGDataProvider:
    ...
    def _normalize_team_name(self, name: str) -> str:
        ...
        name = re.sub(r'\s*(fc|cf|sc|afc|ac|as|ss|us|cd|ud|rc|rcd|real|athletic|atletico)\s*', ' ', name)
    ...
    def _estimate_xg_from_goals(
                ...
                try:
                    ...
                    for match in matches:
                        ...
                        if self._normalize_team_name(home) == team_key:
                            home_goals = match.get('home_goals', match.get('score', {}).get('fullTime', {}).get('home'))
                            away_goals = match.get('away_goals', match.get('score', {}).get('fullTime', {}).get('away'))
        ...
        recent_goals_for = sum(goals_for[-5:]) if len(goals_for) >= 5 else sum(goals_for)
        recent_goals_against = sum(goals_against[-5:]) if len(goals_against) >= 5 else sum(goals_against)
...
class XGPredictionAdjuster:
    ...
    def adjust_prediction(
        ...
        if abs(home_overperformance) > 2 or abs(away_overperformance) > 2:
            ...
            if home_overperformance > 2:
                luck_warning.append(f"{home_team} overperforming xG by {home_overperformance:.1f}")
            ...
            if home_overperformance < -2:
                luck_warning.append(f"{home_team} underperforming xG by {-home_overperformance:.1f}")
                # They might be due for positive regression
                result['home_prob'] = min(result['home_prob'] + 2, 80)
                
            if away_overperformance > 2:
                luck_warning.append(f"{away_team} overperforming xG by {away_overperformance:.1f}")
                result['away_prob'] = max(result['away_prob'] - 2, 10)
                result['confidence'] = min(result['confidence'] - 3, 80)
                
            if away_overperformance < -2:
                luck_warning.append(f"{away_team} underperforming xG by {-away_overperformance:.1f}")
    ...
    def _calculate_xg_expected_goals(
        ...
        if team_stats.xg_overperformance > 0:
            ...
            regression = abs(team_stats.xg_overperformance) * self.overperformance_regression
            xg_rate += regression / team_stats.matches_played
...
if __name__ == "__main__":
    ...
    print(f"  Adjusted probs: {result['home_prob']}/{result['draw_prob']}/{result['away_prob']}")
