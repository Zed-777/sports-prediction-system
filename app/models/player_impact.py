...
class PlayerImpactCalculator:
    ...
    def calculate_impact(self, player: PlayerStats) -> float:
        ...
        weights = self.POSITION_WEIGHTS.get(
            player.position, 
            self.POSITION_WEIGHTS[PlayerPosition.MIDFIELDER]
        )
        ...
        if player.position == PlayerPosition.FORWARD:
            ...
            goal_factor = min(2.0, goals_per_90 / self.LEAGUE_BENCHMARKS['goals_per_90'])
            assist_factor = min(2.0, assists_per_90 / self.LEAGUE_BENCHMARKS['assists_per_90'])
            xg_factor = min(2.0, xg_per_90 / self.LEAGUE_BENCHMARKS['xg_per_90'])
            xa_factor = min(2.0, xa_per_90 / self.LEAGUE_BENCHMARKS['xa_per_90'])
            sot_factor = min(2.0, (player.shots_on_target / minutes_90s) / 1.5) if minutes_90s > 0 else 0
            ...
            goal_factor = min(2.0, goals_per_90 / (self.LEAGUE_BENCHMARKS['goals_per_90'] * 0.7))
            assist_factor = min(2.0, assists_per_90 / self.LEAGUE_BENCHMARKS['assists_per_90'])
            xg_factor = min(2.0, xg_per_90 / (self.LEAGUE_BENCHMARKS['xg_per_90'] * 0.7))
            xa_factor = min(2.0, xa_per_90 / self.LEAGUE_BENCHMARKS['xa_per_90'])
            bcc_factor = min(2.0, (player.big_chances_created / minutes_90s) / 0.3) if minutes_90s > 0 else 0
            ...
            cs_factor = min(2.0, (player.clean_sheets / player.matches_played) / self.LEAGUE_BENCHMARKS['clean_sheets_per_game']) if player.matches_played > 0 else 0
            tackles_factor = min(2.0, (player.tackles / minutes_90s) / self.LEAGUE_BENCHMARKS['tackles_per_90']) if minutes_90s > 0 else 0
            int_factor = min(2.0, (player.interceptions / minutes_90s) / self.LEAGUE_BENCHMARKS['interceptions_per_90']) if minutes_90s > 0 else 0
            blocks_factor = min(2.0, (player.blocks / minutes_90s) / 1.0) if minutes_90s > 0 else 0
            clear_factor = min(2.0, (player.clearances / minutes_90s) / 4.0) if minutes_90s > 0 else 0
            ...
            cs_factor = min(2.0, (player.clean_sheets / player.matches_played) / self.LEAGUE_BENCHMARKS['clean_sheets_per_game']) if player.matches_played > 0 else 0
            saves_factor = min(2.0, (player.saves / minutes_90s) / self.LEAGUE_BENCHMARKS['saves_per_90']) if minutes_90s > 0 else 0
            mins_factor = min(1.5, player.minutes_played / (38 * 90))
...
class PlayerImpactAdjuster:
    ...
    def calculate_adjustment(
        ...
        if unavailable_players is None:
            impact_loss, missing_info = self.database.calculate_team_impact_loss(team_id)
        else:
        ...
        for player in missing_info:
            ...
            impacts = self.POSITION_GOAL_IMPACT.get(pos, {'for': -0.05, 'against': 0.05})
    ...
    def adjust_win_probability(
        ...
        if home_impact.get('missing_players'):
            ...
            reasons.append(f"home without {top_missing['name']} ({top_missing['impact']:.0f} impact)")
        
        if away_impact.get('missing_players'):
            top_missing = away_impact['missing_players'][0]
            reasons.append(f"away without {top_missing['name']} ({top_missing['impact']:.0f} impact)")
...
class PlayerImpactSuite:
    ...
    def analyze_match_impact(
        ...
        return {
            ...
            'analysis_applied': home_impact['adjustment_applied'] or away_impact['adjustment_applied']
        }
    ...
    def register_player(
        ...
        pos = PlayerPosition(position) if position in [p.value for p in PlayerPosition] else PlayerPosition.UNKNOWN
...
if __name__ == '__main__':
    ...
    haaland = suite.register_player(
        ...
        position="FWD",
        stats={
            'matches': 30, 'minutes': 2500, 'starts': 28,
            'goals': 28, 'assists': 5, 'xg': 24.5, 'xa': 3.2,
            'shots': 95, 'shots_on_target': 55
        }
    )
    ...
    kdb = suite.register_player(
        ...
        position="MID",
        stats={
            'matches': 25, 'minutes': 2000, 'starts': 22,
            'goals': 5, 'assists': 15, 'xg': 4.2, 'xa': 12.5,
    ...
    dias = suite.register_player(
        ...
        position="DEF",
        stats={
            'matches': 28, 'minutes': 2450, 'starts': 27,
            'goals': 2, 'clean_sheets': 12,
            'tackles': 45, 'interceptions': 35, 'blocks': 25, 'clearances': 120
        }
    )
    ...
    salah = suite.register_player(
        ...
        position="FWD",
        stats={
            'matches': 32, 'minutes': 2800, 'starts': 31,
            'goals': 22, 'assists': 12, 'xg': 19.5, 'xa': 9.2,
            'shots': 110, 'shots_on_target': 52
        }
    )
    ...
    result = suite.analyze_match_impact(
        ...
        draw_prob=0.28,
        away_prob=0.27,
        home_unavailable=[{'name': 'Erling Haaland', 'position': 'FWD', 'impact': 85.0, 'status': 'injured'}]
    )
