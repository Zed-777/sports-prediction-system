...
class PredictionRecord:
    ...
    probability_for_outcome: Optional[float] = None  # Prob we assigned to actual outcome
...
class PredictionTracker:
    ...
    def _init_database(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_result ON predictions(actual_outcome)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_correct ON predictions(prediction_correct)")
    ...
    def store_prediction(self, record: PredictionRecord) -> str:
        ...
        """, (
            ...
            1 if record.prediction_correct else (0 if record.prediction_correct is not None else None),
    ...
    def get_accuracy_stats(
        ...
        return {
            ...
            'by_confidence': {
                'high': {
                    'count': len(high_conf),
                    'accuracy': sum(1 for r in high_conf if r.prediction_correct) / len(high_conf) if high_conf else None
                },
                'medium': {
                    'count': len(med_conf),
                    'accuracy': sum(1 for r in med_conf if r.prediction_correct) / len(med_conf) if med_conf else None
                },
                'low': {
                    'count': len(low_conf),
                    'accuracy': sum(1 for r in low_conf if r.prediction_correct) / len(low_conf) if low_conf else None
                }
            }
        }
...
def create_prediction_record(
    ...
    **kwargs
    ...
    if kwargs:
        ...
        home_team_name = kwargs.get('home_team', kwargs.get('home_team_name', 'Unknown'))
        away_team_name = kwargs.get('away_team', kwargs.get('away_team_name', 'Unknown'))
    ...
    return PredictionRecord(
        ...
        predicted_home_prob=prediction.get('home_win_prob', prediction.get('home_win_probability', 33)),
        predicted_draw_prob=prediction.get('draw_prob', prediction.get('draw_probability', 33)),
        predicted_away_prob=prediction.get('away_win_prob', prediction.get('away_win_probability', 34)),
