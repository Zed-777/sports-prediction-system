...
class HistogramBinning:
    ...
    def __init__(self, n_bins: int = 15):
        ...
        self.bin_calibrated: np.ndarray = np.linspace(0.5 / n_bins, 1 - 0.5 / n_bins, n_bins)
        self.is_fitted = False
...
class VennABERS:
    ...
    def calibrate(self, prob: float) -> Tuple[float, float]:
        ...
        pos_0 = next(i for i, (s, l) in enumerate(scores_with_0) if s == prob and l == 0)
        ...
        pos_1 = next(i for i, (s, l) in enumerate(scores_with_1) if s == prob and l == 1)
...
class AdaptiveCalibrator:
    ...
    def record_performance(self, method: str, error: float, context: str = 'default'):
        ...
        if context not in self.context_weights:
            self.context_weights[context] = {m: 1.0 for m in self.method_performances}
        
        # Update weights with exponential moving average
        alpha = 0.1
        for m in self.method_performances:
            if m == method:
                self.context_weights[context][m] = (
                    (1 - alpha) * self.context_weights[context][m] + 
                    alpha * (1 - error)  # Higher weight for lower error
                )
...
class AdvancedCalibrationManager:
    ...
    def add_sample(self, predicted_probs: np.ndarray, true_label: int, 
                   context: str = 'default'):
        """
    ...
    def fit_all(self, min_samples: int = 50) -> bool:
        ...
        try:
            ...
            self.logger.info(f"  ✓ Temperature Scaling fitted (T={self.temperature.temperature:.3f})")
            ...
            self.logger.info(f"  ✓ Conformal Predictor fitted (quantile={self.conformal.quantile:.3f})")
    ...
    def calibrate(self, probs: np.ndarray, 
                  method: str = 'ensemble',
                  context: str = 'default') -> CalibrationResult:
        """
    ...
    def _calculate_reliability(self, probs: np.ndarray, pred_set: List[int], 
    ...
    def get_calibration_metrics(self, probs: np.ndarray = None, 
                                 labels: np.ndarray = None) -> CalibrationMetrics:
        """
        ...
        if probs is None:
            if not self.calibration_data['probs']:
                return CalibrationMetrics(
                    ...
                    brier_score=0.0,
                    log_loss=0.0,
                    reliability_diagram={'bins': [], 'accuracies': [], 'confidences': []},
                    sharpness=0.0
                )
            probs = np.array(self.calibration_data['probs'])
            labels = np.array(self.calibration_data['labels'])
        ...
        log_loss = -np.mean(np.log(clipped_probs[np.arange(len(labels)), labels.astype(int)]))
    ...
    def _save_calibration(self):
        ...
        state = {
            'platt': {'A': float(self.platt.A), 'B': float(self.platt.B), 'fitted': self.platt.is_fitted},
            'temperature': {'T': float(self.temperature.temperature), 'fitted': self.temperature.is_fitted},
            'beta': {'a': float(self.beta.a), 'b': float(self.beta.b), 'c': float(self.beta.c), 'fitted': self.beta.is_fitted},
            'histogram': {
    ...
    def _load_calibration(self):
        ...
        try:
            ...
            self.histogram.bin_edges = np.array(hist.get('bin_edges', np.linspace(0, 1, 16)))
            self.histogram.bin_calibrated = np.array(hist.get('bin_calibrated', np.zeros(15)))
            ...
            self.calibration_data = state.get('calibration_data', {'probs': [], 'labels': [], 'contexts': []})
