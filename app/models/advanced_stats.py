...
class ShotQualityAnalyzer:
    ...
    def analyze_team(
        ...
        goals_scored = (
            home_stats.get('goals_scored', 0) +
            away_stats.get('goals_scored', 0)
        )
        ...
        is_overperforming = deviation > 0.2  # Scoring 0.2+ goals more than expected per game
        ...
        stats = ShotQualityStats(
            ...
            big_chances_missed_per_game=big_chances * 0.35,  # ~35% of big chances missed
...
class DefensiveSolidityAnalyzer:
    ...
    LEAGUE_AVERAGES = {
        ...
        'default': {'conceded': 1.20, 'clean_sheet': 0.28, 'blocks': 3.5, 'tackles': 15.0}
    }
    ...
    def analyze_team(
        ...
        # Clean sheet rate
        clean_sheets = (
            home_stats.get('clean_sheets', 0) +
            away_stats.get('clean_sheets', 0)
        )
        clean_sheet_rate = clean_sheets / max(total_matches, 1)
        
        # Estimate defensive actions
        shots_faced_pg = avgs['conceded'] / 0.10 * (conceded_pg / avgs['conceded']) if avgs['conceded'] > 0 else 12.0
        
        # Calculate defensive rating (0-100)
        # Lower concession = higher rating
        concession_factor = max(0, 1 - (conceded_pg - avgs['conceded']) / 2)
        clean_sheet_factor = clean_sheet_rate / avgs['clean_sheet'] if avgs['clean_sheet'] > 0 else 1
        
        defensive_rating = min(100, max(0, 
...
if __name__ == '__main__':
    ...
    print(f"  Performance: {'OVER' if sq['is_overperforming'] else 'UNDER' if sq['is_underperforming'] else 'NORMAL'}")
    ...
    print(f"\nGoal Adjustments:")
    print(f"  Home: {2.0:.2f} → {result['adjusted_home_goals']:.2f} ({result['home_goals_change']:+.2f})")
    print(f"  Away: {1.0:.2f} → {result['adjusted_away_goals']:.2f} ({result['away_goals_change']:+.2f})")
