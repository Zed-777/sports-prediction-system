...
class ABTestingFramework:
    ...
    def _calculate_variant_metrics(
        ...
        variant_preds = [
            ...
            if p.variant_name == variant_name and p.actual_outcome is not None
        ]
        ...
        # Brier scores
        brier_scores = [
            p.brier_score for p in variant_preds if p.brier_score is not None
        ]
        ...
        high_conf_accuracy = (
            high_conf_correct / len(high_conf_preds) if high_conf_preds else 0
        )
    ...
    def _calculate_significance(
        ...
        # Pooled proportion
        p_pool = (control_correct + treatment_correct) / (
    ...
    def _check_experiment_completion(self, experiment_id: str):
        ...
        control_preds = [
            p for p in completed_preds if p.variant_name == exp.control.name
        ]
        treatment_preds = [
            p for p in completed_preds if p.variant_name == exp.treatment.name
        ]
